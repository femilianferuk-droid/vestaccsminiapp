...
def _create_listing_from_session(
    ...
    ⚠️ FIX «уже купили» (баг #1):
    Раньше при пересоздании объявления по уже существующему номеру телефона
    мы тупо сбрасывали `existing.is_sold = False`. Это приводило к циклу:
      1) аккаунт продан (is_sold=True, Purchase создана)
      2) кто-то снова выставляет его — is_sold=False, новая Listing
      3) каталог снова показывает аккаунт (фильтр is_sold=False)
      4) покупатель жмёт «Купить» → /api/buy видит старую Purchase
         и возвращает already_sold
      5) каталог рефрешится, но is_sold всё ещё False → аккаунт снова
         в списке → тот же already_sold по кругу.

    Теперь:
      • Если по этому аккаунту есть «настоящая» Purchase (от покупателя,
        который не равен текущему продавцу) — отказываемся пересоздавать
        объявление и возвращаем None. Вызывающий получит ошибку
        already_sold_for_seller и покажет пользователю вменяемое
        сообщение «этот аккаунт уже куплен, перепродажа невозможна».
      • Иначе — безопасно сбрасываем is_sold=False, гасим все
        listings в статусах sold/cancelled и создаём новый active.
    """
    ...
    if existing:
        # Есть ли «реальный» покупатель, не равный текущему продавцу?
        # Используем .first(), а не scalar_one_or_none() — на случай,
        # если по какой-то причине записей Purchase больше одной.
        sold_to = session.execute(
            select(Purchase).where(
                Purchase.account_id == existing.id,
                Purchase.user_id != int(seller_id),
            )
        ).first()
        if sold_to is not None:
            # Аккаунт уже продан кому-то другому — пересоздавать объявление
            # НЕЛЬЗЯ. Это и есть корень бага «уже купили».
            app.logger.warning(
                "Refuse to re-sell account %s (id=%s): already sold to %s, "
                "current seller is %s",
                phone, existing.id,
                sold_to[0].user_id if hasattr(sold_to[0], "user_id") else "?",
                seller_id,
            )
        ...
        # Безопасно: реальных покупок нет → можно перевыставить.
        ...
        # Заодно подчистим «зависшие» listings в статусах sold/cancelled —
        # они нам не нужны, новый объявление будет active.
        stale = session.execute(
            select(Listing).where(
                Listing.account_id == existing.id,
                Listing.status.in_(("sold", "cancelled")),
            )
        ).scalars().all()
        for SL in stale:
            SL.status = "cancelled"
...
def api_sell_account_session(telegram_id, tg_user):
    ...
    try:
        ...
        if listing is None:
            # Аккаунт с этим телефоном уже продан другому человеку —
            # пересоздавать объявление нельзя (см. FIX «уже купили»).
            try:
                session.rollback()
            except Exception:
                pass
            return jsonify({
                "ok": False,
                "error": "account_already_sold",
                "detail": "Этот аккаунт уже куплен другим пользователем. "
                          "Перепродажа невозможна.",
            }), 409
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        .item-seller-avatar {
            ...
            overflow: hidden;       /* чтобы <img> с object-fit:cover не вылезал */
        }
        /* Когда в .item-seller-avatar лежит <img> (а не буква) — растягиваем
        его на весь кружок. Иначе аватарка из Telegram будет маленькой иконкой
        в верхнем углу. */
        ...
        .page-help-spacer { width: 40px; height: 40px; }
        .page-help-sub {
            font-size: 14px;
        ...
        .help-card[open] .help-summary { border-bottom: 1px solid var(--gray-100, #f1f5f9); }
        .help-emoji { font-size: 22px; line-height: 1; flex-shrink: 0; }
        .help-title-text {
            flex: 1;
            font-weight: 600;
            font-size: 15px;
...
<body>
    ...
    <!-- ===== Страница «Помощь» =====
         Отдельная вкладка (без support). Внутри — советы и FAQ.
         Кнопка «Связаться с поддержкой» внизу открывает модалку
         Support (старая логика, ничего не удалено). -->
    <div class="page" id="pageHelp" aria-hidden="true">
        <div class="page-help-head">
            <button class="profile-back" id="helpBack" aria-label="Назад">
                <span aria-hidden="true">←</span>
            </button>
            <h2 class="page-help-title">Помощь</h2>
            <div class="page-help-spacer"></div>
        </div>
        <div class="page-help-sub">
            Собрали короткие советы — чтобы покупка и продажа проходили без сюрпризов.
        </div>
        ...
        <div class="help-list">
            <details class="help-card" open>
                <summary class="help-summary">
                    ...
                    <span class="help-title-text">Как купить аккаунт</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <ol class="help-steps">
                        <li>Откройте <b>Каталог</b> и выберите страну (или «Все»).</li>
                        <li>Нажмите на карточку — откроются подробности по аккаунту.</li>
                        <li>Пополните баланс (СБП / Crypto / промокод), если нужно.</li>
                        <li>Жмите <b>«Купить»</b> — данные придут сразу после оплаты.</li>
                    </ol>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    <span class="help-emoji">💸</span>
                    <span class="help-title-text">Как продать аккаунт</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <ol class="help-steps">
                        <li>Зайдите в раздел <b>«Продать»</b> через меню.</li>
                        <li>Загрузите <code>.session</code>-файл (или авторизуйтесь по номеру).</li>
                        <li>Укажите название, описание, цену и происхождение.</li>
                        <li>Опубликуйте объявление — оно появится в каталоге.</li>
                        <li>После продажи деньги попадут в <b>холд на 24 ч</b>, потом зачислятся на баланс.</li>
                    </ol>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    <span class="help-emoji">🕓</span>
                    <span class="help-title-text">Что такое холд 24 часа</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <p>После каждой продажи деньги продавца лежат в <b>холде 24 часа</b> — это защита покупателя от мошенничества.</p>
                    <p>Если с аккаунтом всё ок — деньги автоматически зачислятся на ваш баланс (за вычетом комиссии 7%).</p>
                    <p>Если что-то не так — за это время можно открыть спор.</p>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    <span class="help-emoji">💰</span>
                    <span class="help-title-text">Комиссия 7% — за что</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <p>С каждой продажи мы удерживаем <b>7%</b> — это комиссия платформы за поддержку сделок, холд, верификацию сессий и работу чата.</p>
                    <p>Пример: продали за <b>1 000 ₽</b> → после холда получите <b>930 ₽</b>.</p>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    <span class="help-emoji">💳</span>
                    <span class="help-title-text">Как пополнить баланс</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <ul class="help-steps">
                        <li><b>СБП</b> — перевод по номеру, зачисление вручную (проверяет админ).</li>
                        <li><b>Crypto Bot</b> — USDT, зачисление автоматически за 1–2 минуты.</li>
                        <li><b>Промокод</b> — введите код из раздела <b>Профиль → Промокод</b>.</li>
                    </ul>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    ...
                    <span class="help-title-text">Аккаунт не работает — что делать</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <ol class="help-steps">
                        <li>Откройте <b>«Мои покупки»</b> → нужная покупка.</li>
                        <li>Зайдите в чат с продавцом и опишите проблему.</li>
                        <li>Если продавец не отвечает 24 часа — откройте спор (кнопка в системном сообщении).</li>
                        <li>Деньги вернутся на баланс после подтверждения админом.</li>
                    </ol>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    <span class="help-emoji">🔐</span>
                    <span class="help-title-text">Безопасность сессий</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <p>Все аккаунты в каталоге проходят <b>верификацию</b> — мы проверяем сессию перед публикацией.</p>
                    <p>Не передавайте свои <code>.session</code>-файлы и коды из Telegram третьим лицам.</p>
                </div>
            </details>
            ...
            <details class="help-card">
                <summary class="help-summary">
                    <span class="help-emoji">🎁</span>
                    <span class="help-title-text">Промокоды</span>
                    <span class="help-caret" aria-hidden="true">▾</span>
                </summary>
                <div class="help-body">
                    <p>Промокоды выдаются в каналах, за репосты и в розыгрышах.</p>
                    <p>Активируются в <b>Профиль → Промокод</b>. Один код можно применить только один раз.</p>
                </div>
            </details>
        </div>

        <div class="help-cta">
            <div class="help-cta-text">Не нашли ответ?</div>
            <button class="btn-primary" id="helpOpenSupport">Связаться с поддержкой</button>
            <button class="btn-secondary" id="helpOpenChat">Открыть чат с покупателем</button>
        </div>
    </div>
    ...
    <script>
        (function () {
            ...
            const dom = {
                ...
                pageHelp: $('pageHelp'),
                helpBack: $('helpBack'),
                helpOpenSupport: $('helpOpenSupport'),
                helpOpenChat: $('helpOpenChat'),
            ...
            function switchPage(pageId, opts) {
                ...
                const pages = ['pageCatalog', 'pageProfile', 'pagePurchases', 'pageTopup', 'pageSell', 'pageChats', 'pageHelp'];
                pages.forEach((p) => {
            ...
            */
            ...
            // Кеш аватарок продавцов на стороне браузера: seller_id -> url|null.
            // Открыли модалку один раз — дальше моментально из кеша, без
            // повторного запроса к /api/seller_photo. Храним и null,
            // чтобы не переспрашивать продавцов без фото.
            const sellerPhotoCache = new Map();
            // Чтобы 10 быстрых открытий модалок не плодили 10 параллельных
            // запросов к /api/seller_photo, сериализуем fetch'и в одну цепочку.
            let sellerPhotoInflight = null;

            function paintSellerAvatar(wrap, url, fallbackInitial) {
                if (!wrap) return;
                if (url) {
                    const img = document.createElement('img');
                    img.src = url;
                    ...
                    img.referrerPolicy = 'no-referrer';
                    img.loading = 'lazy';
                    // Если Telegram-файл не загрузился (network/CORS) —
                    // возвращаемся к букве-инициалу, чтобы аватарка
                    // не была «битой» навсегда.
                    img.onerror = () => {
                        wrap.innerHTML = '';
                        wrap.textContent = fallbackInitial || '?';
                    };
                    wrap.innerHTML = '';
            ...
            function renderSellerAvatar(sellerId, fallbackInitial) {
                const wrap = document.getElementById('itemSellerAvatar');
                if (!wrap) return;

                // Всегда начинаем с инициала — мгновенный рендер, без мигания.
                wrap.innerHTML = '';
                wrap.textContent = fallbackInitial || '?';

                if (!sellerId) return;  // Платформа — без аватарки.

                // Уже знаем ответ — рисуем сразу.
                if (sellerPhotoCache.has(sellerId)) {
                    paintSellerAvatar(wrap, sellerPhotoCache.get(sellerId), fallbackInitial);
                    return;
                }

                // Иначе ставим запрос в общую очередь (не более 1 в полёте).
                const doFetch = () => api(
                    `/api/seller_photo?seller_id=${encodeURIComponent(sellerId)}`
                ).then((r) => {
                    const url = (r && r.ok && r.data && r.data.url) ? r.data.url : null;
                    sellerPhotoCache.set(sellerId, url);
                    // Перерисуем только если модалка всё ещё открыта и это
                    // тот же seller (юзер мог уже закрыть или открыть другую).
                    const stillOpen = !document.getElementById('itemModal').classList.contains('hidden');
                    if (stillOpen && buyState.item && Number(buyState.item.seller_id) === Number(sellerId)) {
                        paintSellerAvatar(wrap, url, fallbackInitial);
                    }
                }).catch(() => {
                    sellerPhotoCache.set(sellerId, null);  // больше не спрашиваем
                }).finally(() => {
                    if (sellerPhotoInflight === doFetch) sellerPhotoInflight = null;
                });

                if (!sellerPhotoInflight) sellerPhotoInflight = doFetch();
                else sellerPhotoInflight = sellerPhotoInflight.then(doFetch, doFetch);
            }
            ...
            function openItem(it) {
                ...
                // Аватарка продавца из Telegram (через /api/seller_photo).
                // Сначала мгновенно рисуем инициал, потом подменяем на <img>.
                const sid = it.seller_id ? Number(it.seller_id) : 0;
                renderSellerAvatar(sid, sellerInitial);
                ...
                if (dom.itemChatBtn) {
                    if (sid && sid !== ((state.tgUser && state.tgUser.id) || 0)) {
            ...
            async function buyCurrentItem() {
                ...
                try {
                    ...
                    if (r.ok) {
                        ...
                        // FIX v2: «уже продан / не найден» — НЕ ошибка для
                        // пользователя. Закрываем модалку, перезагружаем
                        // каталог и сообщаем в тосте.
                        if (err === 'already_sold' || err === 'not_found') {
                            try { closeModal('itemModal'); } catch (e) {}
                            showToast(
                                err === 'not_found'
                                    ? '♻️ Аккаунт уже недоступен — обновляем каталог...'
                                    : '♻️ Этот аккаунт уже купили — обновляем каталог...',
                                'info'
                            );
                            // Перезагрузим каталог и баланс.
            ...
            function bindEvents() {
                ...
                // Пункт «Помощь» из бокового меню — теперь открывает
                // отдельную вкладку с советами (без support).
                // openSupport() продолжает жить для остальных мест
                // (профиль, кнопка «Спор» в покупках и т.п.).
                ...
                // Кнопка «назад» со страницы «Помощь»
                if (dom.helpBack) {
                    dom.helpBack.addEventListener('click', () => switchPage('pageCatalog'));
                }
                // Со страницы «Помощь» кнопка «Связаться с поддержкой» —
                // та же модалка support, что и в профиле (логика не удалена).
                if (dom.helpOpenSupport) {
                    dom.helpOpenSupport.addEventListener('click', () => {
                        if (typeof openSupport === 'function') openSupport();
                        else showToast('Откройте поддержку через меню', 'info');
                    });
                }
                // Со страницы «Помощь» кнопка «Открыть чат» — идём в раздел Чатов
                if (dom.helpOpenChat) {
                    dom.helpOpenChat.addEventListener('click', () => switchPage('pageChats'));
                }
...
@app.route("/api/seller_photo")
@require_auth
def api_seller_photo(telegram_id, tg_user):
    """Возвращает URL аватарки продавца из Telegram (Bot API).

    Используется в модалке товара («просмотр товара где есть описание и
    кнопка купить»), чтобы показать настоящую аватарку продавца, а не
    просто букву-инициал. Достаём через getUserProfilePhotos → getFile,
    кешируем в памяти процесса на _TG_PHOTO_TTL (6 часов) — второй запрос
    по тому же продавцу мгновенный.

    Отдаём {'ok': True, 'url': str|None}. None = у продавца нет фото
    или Telegram не отдал — фронт в таком случае покажет инициал.
    """
    try:
        ...
        seller_id = 0
    if seller_id <= 0:
        return jsonify({"ok": False, "error": "bad_seller_id"}), 400

    # Не светим чужие аватарки неавторизованным — но @require_auth уже
    # гарантирует, что telegram_id валиден. Просто отдаём URL.
    url = _get_telegram_photo_url(seller_id)
    return jsonify({"ok": True, "seller_id": seller_id, "url": url})
...
def api_catalog():
    ...
    session = SessionLocal()
    try:
        # FIX v2: Не показываем аккаунты с «зависшим» is_sold=True.
        # Если есть запись Account.is_sold=False, но есть активный
        # Listing.status='sold' — пока оставляем (это нормальный кейс:
        # один аккаунт несколько продавцов выставили).
        #
        # Основной фильтр — is_sold=False + is_verified=True (как и был).
...
def api_buy(telegram_id, tg_user):
    ...
    try:
        ...
        # ===== FIX «уже продан» v2 (полный) =====
        # У мини-апа прилечали жалобы «аккаунт уже продан, хотя он не продан».
        # Реальные причины:
        #  1) В БД is_sold=True остался без реальной Purchase (тест / сбой).
        #  2) У аккаунта listing.status='sold' без Purchase-записи.
        #  3) Гонка с ботом — другой пользователь только что купил этот
        #     аккаунт, is_sold стал True, listing стал sold, Purchase есть.
        # Логика:
        #  • Если есть реальная Purchase по этому аккаунту — это и есть факт
        #    продажи. Тогда возвращаем already_sold, ничего не трогаем.
        #  • Иначе (Purchase нет) — сбрасываем is_sold=False и гасим все
        #    listings в статусах sold/cancelled (от прошлых попыток).
        #  • Возвращаем already_sold ТОЛЬКО если есть реальная Purchase.
        #
        # ВАЖНО: используем .first(), а НЕ scalar_one_or_none() — иначе при
        # нескольких Purchase-записях (тестовые / восстановленные) SQLAlchemy
        # выбрасывает MultipleResultsFound → 500 server_error.
        real_purchase = session.execute(
            select(Purchase).where(
                Purchase.account_id == account.id,
                Purchase.user_id != (account.seller_id or 0),
            )
        ).first()
        if real_purchase is not None:
            # Реальная покупка была — честно отдаем already_sold.
            #
            # ⚠️ FIX «уже купили» (bug #1, self-heal):
            # Без этого блока аккаунт вечно торчит в каталоге: фильтр
            # каталога смотрит на Account.is_sold, а Purchase-запись
            # не учитывает. Раньше юзер получал already_sold, каталог
            # рефрешился — а аккаунт всё ещё там, потому что is_sold=False.
            # Теперь чиним state: гасим все активные listings и помечаем
            # аккаунт проданным, чтобы каталог его больше не показывал.
            try:
                if not account.is_sold:
                    app.logger.warning(
                        "Self-heal: account %s has Purchase %s but is_sold=False. "
                        "Fixing catalog state.",
                        account.id, real_purchase[0].id,
                    )
                    account.is_sold = True
                # Гасим активные listings — иначе они будут светиться в
                # P2P-маркетплейсе бота, пока тот не увидит обновление.
                ...
                for SL in stale_active:
                    SL.status = "cancelled"
                if stale_active:
                    app.logger.warning(
                        "Self-heal: cancelled %s stale active listings for account %s",
                        len(stale_active), account.id,
                    )
                session.commit()
            except Exception as heal_err:
                app.logger.warning("Self-heal failed (non-fatal): %s", heal_err)
                try:
                    session.rollback()
                except Exception:
                    pass
        ...
        # Если дошли сюда — реальных Purchase нет. Если флаг/листинги
        # говорят что продано — это сбой БД. Чиним.
        if account.is_sold:
            try:
                app.logger.warning(
                    "Resetting orphan is_sold on account %s (no Purchase, was sold)",
                    account.id,
                )
            except Exception:
                pass
        ...
        # Чиним и «зависшие» listings (status='sold' без реальной покупки).
        # Они могут блокировать каталог в полусломанном состоянии.
        # Используем .scalars() — он возвращает список, без падений на длине.
        stale_listings_q = session.execute(
            select(Listing).where(
                Listing.account_id == account.id,
                Listing.status.in_(("sold", "cancelled")),
            )
        ).scalars()
        for SL in stale_listings_q:
            SL.status = "active"  # восстанавливаем, чтобы можно было купить
        ...
        if account.seller_id:
            ...
            if seller:
                ...
                new_hold = Hold(
                    ...
                    listing_id=None,  # покупка напрямую из каталога
                    purchase_id=None,  # заполним ниже после flush
                ...
                session.flush()
                new_hold.purchase_id = purchase.id
