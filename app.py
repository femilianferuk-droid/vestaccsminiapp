...
<body>
    ...
    <div class="page" id="pageChats">
        <div class="topup-hero">
            <button class="profile-back" id="chatsBack" aria-label="Назад">←</button>
        ...
        (function () {
            ...
            function renderChats() {
                ...
                for (const c of chatsState.items) {
                    ...
                    const display = c.peer_username
                        ? '@' + c.peer_username
                        : 'id ' + c.peer_id;
                    const initial = chatInitial(display);
                    const time = formatChatTime(c.last_message_at);
                    const preview = c.last_message
                        ? (c.last_message_sender_id === (state.tgUser && state.tgUser.id) ? 'Вы: ' : '') + c.last_message
                        : 'Нет сообщений';
                    ...
                    card.innerHTML = `
                        ...
                        <div class="chat-card-body">
                            <div class="chat-card-avatar">${escapeHtml(initial)}</div>
            ...
            async function openChat(c) {
                ...
                // Шапка модалки
                const display = c.peer_username ? '@' + c.peer_username : 'id ' + c.peer_id;
                if (dom.chatModalName) dom.chatModalName.textContent = display;
                if (dom.chatModalSub) dom.chatModalSub.textContent = 'telegram id ' + c.peer_id;
                if (dom.chatModalAvatar) dom.chatModalAvatar.textContent = chatInitial(display);
            ...
            // счётчик непрочитанных (раз в 3 сек), чтобы бейдж был живой,
            // но без лишней нагрузки на БД.
            function startChatsPolling() {
                setInterval(() => {
                    const chatsOpen = dom.chatsModal && dom.chatsModal.classList.contains('open');
                    const chatOpen = dom.chatModal && dom.chatModal.classList.contains('open');
                    if (chatsOpen || chatOpen) {
                        // Открыта одна из модалок — обновляем всё
                        if (chatsOpen) {
                            pollChats();
                        }
                        if (chatOpen && chatsState.activePeerId) {
                            pollActiveMessages();
                        }
                        refreshUnreadBadge();
                    } else {
                        // Ничего не открыто — обновляем только бейдж
                        refreshUnreadBadge();
                    }
                }, 500);
            }

            // Тихий опрос активного диалога: не мигает, не скроллит,
            // только подтягивает новые сообщения и обновляет read_at у
            // отправленных нами (чтобы галочки посинели, как только
            // собеседник открыл диалог).
            ...
            async function openChatByPeerId(peerId, peerMeta) {
                ...
                const display = peerMeta && peerMeta.username
                    ? '@' + peerMeta.username
                    : 'id ' + peerId;
                openChat({
                    peer_id: peerId,
                    peer_username: peerMeta && peerMeta.username || null,
                    unread: 0,
                    last_message: null,
...
def api_buy(telegram_id, tg_user):
    ...
    try:
        ...
        # сообщение о покупке. Так покупатель и продавец сразу видят сделку
        # в своих чатах, могут обсудить детали и т.п.
        chat_thread_id = None
        if account.seller_id and int(account.seller_id) != int(telegram_id):
            try:
                ...
                # Отправитель — покупатель (sender_id=telegram_id), чтобы
                # в чате оно сразу подсветилось как входящее у продавца.
                sys_text = (
    ...
    try:
        ...
        for t in threads:
            ...
            chats.append({
                ...
                "peer_first_name": (
                    (tg_user if False else None)  # заглушка; реальное имя берём из initData
                ),
