"""
Маркетплейс — однофайловое Flask-приложение.
...
from sqlalchemy import (
    ...
    create_engine, func, literal_column, select, text,
)
...
# ===== БОТ ПОДДЕРЖКИ (ОТДЕЛЬНОЕ «ЛИЦО» В ЧАТЕ) =====
#
# В in-app чате между покупателем и продавцом сообщения о покупке и
# релизе холда пишет «Поддержка» — отдельное лицо, а не покупатель.
...
# из PNG-файла в репозитории и именем «Поддержка».
BOT_SENDER_ID = 0
...
class MarketItem(Base):
    """Generic marketplace item for non-account categories."""
    __tablename__ = "market_items"
    id = Column(Integer, primary_key=True)
    seller_id = Column(BigInteger, nullable=False, index=True)
    game = Column(String(40), nullable=False, default="telegram", index=True)
...
class MarketOrder(Base):
    """Generic item order with buyer receipt confirmation."""
    __tablename__ = "market_orders"
    id = Column(Integer, primary_key=True)
    buyer_id = Column(BigInteger, nullable=False, index=True)
    item_id = Column(Integer, nullable=False, index=True)
    amount = Column(Float, nullable=False)
    status = Column(String(30), default="awaiting_confirmation")
    created_at = Column(DateTime, default=datetime.utcnow)
    confirmed_at = Column(DateTime, nullable=True)


class MarketHold(Base):
    __tablename__ = "market_holds"
    id = Column(Integer, primary_key=True)
    seller_id = Column(BigInteger, nullable=False)
...
_LISTING_PIN_SCHEMA_READY = False
_LISTING_PIN_SCHEMA_LOCK = threading.Lock()
_PURCHASE_CONFIRM_SCHEMA_READY = False
_PURCHASE_CONFIRM_SCHEMA_LOCK = threading.Lock()


def _ensure_listing_pin_columns() -> bool:
    """Лениво добавляет поля закрепления, не меняя ORM-схему общего бота.

    bot.py продолжает работать со старым набором полей Listing, а мини-апп
    читает новые колонки точечными SQL-выражениями. Это не заставляет все
    существующие SELECT по listings зависеть от успешного запуска миграции.
    """
    global _LISTING_PIN_SCHEMA_READY
    if _LISTING_PIN_SCHEMA_READY:
        return True
    with _LISTING_PIN_SCHEMA_LOCK:
        if _LISTING_PIN_SCHEMA_READY:
            return True
        try:
            with engine.begin() as conn:
                if engine.dialect.name == "sqlite":
                    ...
                    if "pin_fee" not in columns:
                        conn.exec_driver_sql(
                            "ALTER TABLE listings ADD COLUMN "
                            "pin_fee FLOAT NOT NULL DEFAULT 0"
                        )
                else:
                    conn.exec_driver_sql(
                    ...
                    conn.exec_driver_sql(
                        "ALTER TABLE listings ADD COLUMN IF NOT EXISTS "
                        "pin_fee DOUBLE PRECISION NOT NULL DEFAULT 0"
                    )
            _LISTING_PIN_SCHEMA_READY = True
            return True
        except Exception as _e:
            try:
                app.logger.warning("listings pin migration failed: %s", _e)
            except Exception:
                pass
            return False


def _ensure_purchase_confirmation_columns() -> bool:
    """Adds confirmation fields to purchases without changing bot.py's ORM.

    Existing purchases remain waiting for confirmation by default.  The
    columns are deliberately accessed with small SQL expressions below so an
    older bot process can continue using the shared table unchanged.
    """
    global _PURCHASE_CONFIRM_SCHEMA_READY
    if _PURCHASE_CONFIRM_SCHEMA_READY:
        return True
    with _PURCHASE_CONFIRM_SCHEMA_LOCK:
        if _PURCHASE_CONFIRM_SCHEMA_READY:
            return True
        try:
            with engine.begin() as conn:
                if engine.dialect.name == "sqlite":
                    ...
                    if "status" not in columns:
                        conn.exec_driver_sql(
                            "ALTER TABLE purchases ADD COLUMN status "
                            "VARCHAR(20) NOT NULL DEFAULT 'awaiting_confirmation'"
                        )
            ...
            _PURCHASE_CONFIRM_SCHEMA_READY = True
            return True
        except Exception as _e:
            try:
                app.logger.warning("purchases confirmation migration failed: %s", _e)
            except Exception:
                pass
            return False
...
def _ensure_market_tables():
    """Lazily creates generic marketplace tables used by new categories."""
    try:
        Base.metadata.create_all(
            bind=engine,
            tables=[MarketItem.__table__, MarketOrder.__table__, MarketHold.__table__],
            checkfirst=True,
        )
        return True
    except Exception as _e:
        try:
            app.logger.warning("market tables DDL failed: %s", _e)
        except Exception:
            pass
        return False
...
MARKET_GAME_CATEGORIES = {
    "telegram": ("accounts", "channels", "services", "boosting"),
    "chatgpt": ("accounts", "subscriptions", "api"),
}
MARKET_CATEGORY_LABELS = {
    "accounts": "Аккаунты", "channels": "Каналы", "services": "Услуги",
    "boosting": "Накрутка", "subscriptions": "Подписки", "api": "API",
}
...
def _get_cached_telegram_photo_url(tg_id: int) -> Optional[str]:
    """Returns a previously fetched photo without doing network I/O.

    Catalog responses must never wait on Telegram's Bot API.  The first
    catalog paint can use the initials fallback; profile screens may still
    fetch a fresh photo on demand through _get_telegram_photo_url().
    """
    if not tg_id:
        return None
    cached = _TG_PHOTO_CACHE.get(int(tg_id))
    if not cached:
        return None
    if datetime.now().timestamp() - cached.get("ts", 0) >= _TG_PHOTO_TTL:
        return None
    return cached.get("url")
...
PIN_FEE_PERCENT = 10.0      # разовая стоимость закрепления от цены лота
...
class InsufficientPinBalance(Exception):
    def __init__(self, required: float, balance: float):
        self.required = required
        self.balance = balance
        super().__init__("insufficient balance for listing pin")
...
def _listing_pin_fee(price: float, is_pinned: bool) -> float:
    if not is_pinned:
        return 0.0
    return round(float(price or 0.0) * PIN_FEE_PERCENT / 100.0, 2)


def _lock_seller_and_charge_pin(
    session, telegram_id: int, tg_user: dict, price: float, is_pinned: bool,
) -> tuple:
    """Блокирует баланс продавца и списывает разовую плату за закрепление."""
    ...
    pin_fee = _listing_pin_fee(price, is_pinned)
    balance = round(float(db_user.balance or 0.0), 2)
    if pin_fee > balance:
        raise InsufficientPinBalance(pin_fee, balance)
    if pin_fee:
        db_user.balance = round(balance - pin_fee, 2)
    return db_user, pin_fee


def _pin_balance_error(exc: InsufficientPinBalance):
    return jsonify({
        "ok": False,
        "error": "low_pin_balance",
        "detail": (
            f"Для закрепления нужно {exc.required:.2f} ₽, "
            f"на балансе {exc.balance:.2f} ₽"
        ),
...
def _check_pin_balance_preview(telegram_id: int, price: float, is_pinned: bool):
    """Ранняя UX-проверка до отправки Telegram-кода; финальная — под lock."""
    if not is_pinned:
        return None
    required = _listing_pin_fee(price, True)
    session = SessionLocal()
    try:
        db_user = session.execute(
            select(User).where(User.telegram_id == telegram_id)
        ).scalar_one_or_none()
        balance = round(float(db_user.balance or 0.0), 2) if db_user else 0.0
        if required > balance:
            return InsufficientPinBalance(required, balance)
        return None
    finally:
        session.close()
...
def _create_listing_from_session(
    ...
    is_pinned: bool = False, pin_fee: float = 0.0,
) -> tuple:
    ...
    session.flush()
    if is_pinned:
        if not _ensure_listing_pin_columns():
            raise RuntimeError("listing pin schema is unavailable")
        session.execute(
            text(
                "UPDATE listings SET is_pinned = :is_pinned, pin_fee = :pin_fee "
                "WHERE id = :listing_id"
            ),
            {
                "is_pinned": True,
                "pin_fee": float(pin_fee or 0.0),
...
def api_sell_account_session(telegram_id, tg_user):
    ...
    if is_pinned and not _ensure_listing_pin_columns():
        return jsonify({"ok": False, "error": "pin_unavailable",
                        "detail": "Закрепление временно недоступно"}), 503
    ...
    session = SessionLocal()
    try:
        db_user, pin_fee = _lock_seller_and_charge_pin(
            session, telegram_id, tg_user, price, is_pinned
        )
        ...
        listing, account = _create_listing_from_session(
            ...
            is_pinned=is_pinned, pin_fee=pin_fee,
        )
        session.commit()
    ...
    except InsufficientPinBalance as e:
        session.rollback()
        return _pin_balance_error(e)
    except Exception as e:
        try:
            session.rollback()
        except Exception:
...
def api_sell_account_session_file(telegram_id, tg_user):
    ...
    is_pinned   = _as_bool(request.form.get("is_pinned"))
    try:
...
def api_sell_account_phone_start(telegram_id, tg_user):
    ...
    draft_is_pinned = _as_bool(draft.get("is_pinned"))
    try:
        draft_price = float(draft.get("price") or 0.0)
    except (TypeError, ValueError):
        draft_price = 0.0
    if draft_is_pinned and not _ensure_listing_pin_columns():
        return jsonify({"ok": False, "error": "pin_unavailable",
                        "detail": "Закрепление временно недоступно"}), 503
    pin_error = _check_pin_balance_preview(
        telegram_id, draft_price, draft_is_pinned
    )
    if pin_error:
        return _pin_balance_error(pin_error)
...
def api_sell_account_phone_verify(telegram_id, tg_user):
    ...
    try:
        ...
        is_pinned = _as_bool(draft.get("is_pinned"))
...
def api_my_listings(telegram_id, tg_user):
    ...
    pin_schema_ready = _ensure_listing_pin_columns()
    session = SessionLocal()
    try:
        listings_query = (
            select(Listing)
        ...
        if pin_schema_ready:
            listings_query = listings_query.add_columns(
                literal_column(
                    "COALESCE(listings.is_pinned, FALSE)"
                ).label("listing_is_pinned"),
                literal_column(
                    "COALESCE(listings.pin_fee, 0)"
                ).label("listing_pin_fee"),
            )
            listing_rows = session.execute(listings_query).all()
        else:
            listing_rows = [
                (item, False, 0.0)
                for item in session.execute(listings_query).scalars().all()
            ]

        items = []
        for l, is_pinned, pin_fee in listing_rows:
...
<head>
    ...
    <title>Маркетплейс аккаунтов</title>
    <!-- The application reads Telegram.WebApp while the page is being parsed,
         so the SDK must be available before the inline application script runs. -->
    <script src="https://telegram.org/js/telegram-web-app.js"></script>
    <style>
        :root {
            /* Палитра маркетплейса: глубокий индиго + бирюза */
            --blue-50:  #eef4ff;
            --blue-100: #d9e6ff;
            --blue-200: #b8d0ff;
            --blue-300: #8db1ff;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        .hero-title .hero-shimmer {
            background: linear-gradient(110deg, var(--blue-600) 20%, #7c3aed 42%, #22c1c3 52%, var(--blue-600) 74%);
        ...
        @keyframes heroShimmer { 0%, 100% { background-position: 100% 0; } 50% { background-position: 0 0; } }
        .hero-sub {
        ...
        .game-market-head { display:flex; align-items:center; gap:10px; margin-bottom:8px; }
        .game-market-label { font-size:12px; font-weight:800; color:var(--text-muted); text-transform:uppercase; letter-spacing:.08em; }
        .game-search { flex:1; min-width:0; border:1px solid var(--gray-200); border-radius:11px; padding:8px 11px; background:var(--surface); color:var(--text); font:inherit; font-size:13px; }
        .game-tabs { display:flex; gap:7px; margin-bottom:10px; overflow:auto; scrollbar-width:none; }
        .game-tabs::-webkit-scrollbar { display:none; }
        .game-tab { border:1px solid var(--gray-200); background:var(--surface); color:var(--text-muted); border-radius:10px; padding:8px 14px; font:inherit; font-size:12px; font-weight:750; cursor:pointer; white-space:nowrap; }
        .game-tab.active { background:var(--text); border-color:var(--text); color:var(--surface); }
        .market-category {
            min-width: 0; min-height: 72px; padding: 10px 8px;
            display: flex; flex-direction: column; align-items: center;
            justify-content: center; gap: 5px; border-radius: 16px;
        ...
        .market-category-icon { font-size: 22px; line-height: 1; }
        .market-category.active {
            color: #fff; border-color: transparent;
            background: linear-gradient(135deg, var(--brand), var(--brand-dark));
            box-shadow: 0 7px 18px rgba(91,61,240,.20);
        }
        .category-description {
            margin-top: 10px; padding: 10px 12px; border-radius: 12px;
            background: var(--blue-50); color: var(--text-muted);
            font-size: 12px; line-height: 1.4;
        }
        .category-empty {
            padding: 38px 22px; text-align: center; border: 1px dashed var(--gray-200);
            border-radius: 20px; background: var(--surface);
        }
        .category-empty-icon { font-size: 38px; margin-bottom: 10px; }
        .category-empty strong { display: block; color: var(--text); font-size: 17px; }
        .category-empty p { margin: 8px 0 0; color: var(--text-muted); line-height: 1.5; font-size: 13px; }
        .category-filter-hint { color: var(--text-muted); font-size: 12px; line-height: 1.45; margin-top: 10px; }
        .market-card-kicker { font-size:11px; font-weight:800; color:var(--brand); text-transform:uppercase; letter-spacing:.06em; margin-bottom:8px; }
        .market-card-desc { color:var(--text-muted); font-size:12px; line-height:1.45; margin:9px 0 12px; display:-webkit-box; -webkit-line-clamp:3; -webkit-box-orient:vertical; overflow:hidden; }
        .generic-sell-card { border:1px solid rgba(91,61,240,.20); }
        .sell-mode-switch { display:flex; gap:6px; padding:4px; margin:0 0 16px; background:var(--gray-100); border-radius:12px; }
        .sell-mode-switch-btn { flex:1; border:0; border-radius:9px; padding:9px 8px; background:transparent; color:var(--text-muted); font:inherit; font-size:12px; font-weight:750; cursor:pointer; }
        .sell-mode-switch-btn.active { background:var(--surface); color:var(--text); box-shadow:var(--shadow-sm); }
        .generic-sell-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:8px; }
        .generic-sell-grid .sell-label { min-width:0; }
        @media (max-width: 520px) { .generic-sell-grid { grid-template-columns:1fr; } }
        .section-head {
        ...
        .card.pinned {
            background: linear-gradient(145deg, #fff7ed 0%, #fffbeb 72%, #fff 100%);
            border-color: #fdba74;
            box-shadow: 0 10px 28px rgba(249,115,22,.13);
        }
        .card.pinned:hover {
            border-color: #f97316;
        ...
        .card-pin-badge {
            ...
            box-shadow: 0 5px 12px rgba(194,65,12,.22);
        }
        .card.pinned .card-title-block { padding-right: 88px; }
        .card.pinned .card-premium-badge { top: 43px; }
        .pin-inline-badge {
        ...
        .loader { display: flex; flex-direction:column; align-items:center; gap:9px; justify-content: center; padding: 32px 0; color:var(--text-muted); font-size:12px; font-weight:700; }
        .spinner {
        ...
        .pur-btn.confirm {
            ...
            color: #fff; box-shadow: 0 4px 14px rgba(217, 119, 6, 0.30);
        }
        .pur-btn.confirm:hover { box-shadow: 0 6px 18px rgba(217, 119, 6, 0.42); }
        .pur-btn.danger {
        ...
        .purchase-confirm-inline { margin-top: 10px; padding: 9px 12px; font-size: 12px; }
        ...
        .profile-listing-card.pinned {
            border-color: #fdba74;
            background: linear-gradient(145deg, #fff7ed, var(--surface));
        }
        .profile-listing-card.pinned .profile-listing-name { padding-right: 82px; }
        [data-theme="dark"] .profile-listing-card.pinned {
            border-color: #7c4a24;
            background: linear-gradient(145deg, #2a211b, var(--surface));
        }
        ...
        .sell-pin-option:hover { border-color: #fb923c; }
        .sell-pin-option:active { transform: scale(.99); }
        .sell-pin-option:has(input:checked) {
            border-color: #f97316;
            box-shadow: 0 8px 20px rgba(249,115,22,.14);
        }
        .sell-pin-option input { position: absolute; opacity: 0; pointer-events: none; }
        .sell-pin-check {
            display: inline-flex; align-items: center; justify-content: center;
            width: 23px; height: 23px; flex: 0 0 23px;
            border: 2px solid #fdba74; border-radius: 7px;
            background: #fff; color: transparent; font-weight: 900;
            transition: all .18s;
        }
        .sell-pin-option input:checked + .sell-pin-check {
            background: #f97316; border-color: #f97316; color: #fff;
        }
        .sell-pin-option input:focus-visible + .sell-pin-check {
            outline: 3px solid rgba(249,115,22,.25); outline-offset: 2px;
        }
        .sell-pin-copy { display: grid; gap: 2px; min-width: 0; }
        .sell-pin-copy strong { font-size: 13.5px; color: #9a3412; }
        .sell-pin-copy small { color: #9a3412; opacity: .78; line-height: 1.35; }
        .sell-pin-copy .sell-pin-fee { margin-top: 3px; opacity: 1; font-weight: 750; }
        .sell-origin-grid {
            display: grid;
            grid-template-columns: repeat(2, 1fr);
            gap: 8px;
        ...
        [data-theme="dark"] .sell-my-item.pinned {
            border-color: #7c4a24;
            background: linear-gradient(145deg, #2a211b, var(--surface));
        }
        .sell-my-item .mi-row {
        ...
        .hero-title span:not(.hero-shimmer) { background: none; -webkit-text-fill-color: var(--brand); color: var(--brand); }
        .hero-title .hero-shimmer { -webkit-text-fill-color: transparent; color: transparent; }
        ...
        /* Единственная тёмная тема включается переключателем; системная тема не меняет интерфейс автоматически. */
        [data-theme="dark"] {
        ...
        [data-theme="dark"] .item-description {
            background: linear-gradient(145deg, #172235, #142033);
            border-color: #2b3b55;
            color: #bac7d9;
            box-shadow: inset 0 1px 0 rgba(148,163,184,.05);
        }
        [data-theme="dark"] .item-description-title { color: #8fb7ea; }
        [data-theme="dark"] .item-description-title .item-description-icon {
            background: #20375b; color: #a9c9f2;
        }
        [data-theme="dark"] .item-description-body,
        [data-theme="dark"] .item-description-grid .desc-row { color: #b8c4d4; }
        [data-theme="dark"] .item-description-body strong,
        [data-theme="dark"] .item-description-grid .desc-row strong { color: #d6deea; }
        [data-theme="dark"] .item-description-grid { border-top-color: #33445e; }
        [data-theme="dark"] .card.pinned {
            background: linear-gradient(145deg, #2a211b, #211d1b 72%, #182133);
            border-color: #9a5b27;
            box-shadow: 0 10px 28px rgba(0,0,0,.24);
        }
        [data-theme="dark"] .sell-pin-option {
            background: linear-gradient(145deg, #2a211b, #211d1b);
            border-color: #7c4a24; color: #fdba74;
        }
        [data-theme="dark"] .sell-pin-copy strong,
        [data-theme="dark"] .sell-pin-copy small { color: #fdba74; }
        [data-theme="dark"] .sell-pin-check { background: #121b2b; border-color: #9a5b27; }
        ...
        [data-theme="dark"] .topup-btn { background: var(--blue-50); }
...
<body>
    ...
    <div class="page active" id="pageCatalog">
        ...
        <section class="hero">
            <h1 class="hero-title"><span class="hero-shimmer">Маркетплейс</span><br><span>аккаунтов</span></h1>
            <p class="hero-sub">Проверенные сессии · моментальная выдача · поддержка 24/7</p>
        </section>

        <section class="section">
            <div class="game-market-head">
                <div class="game-market-label">Игра</div>
                <input class="game-search" id="gameSearch" type="search" placeholder="Поиск игры" autocomplete="off">
            </div>
            <div class="game-tabs" id="gameTabs" role="tablist" aria-label="Игры">
                <button class="game-tab active" type="button" data-game="telegram" role="tab" aria-selected="true">Telegram</button>
                <button class="game-tab" type="button" data-game="chatgpt" role="tab" aria-selected="false">ChatGPT</button>
            </div>
            <div class="market-categories" id="marketCategories" role="tablist" aria-label="Категории маркетплейса">
                <button class="market-category active" type="button" role="tab" aria-selected="true" data-category="accounts">
                    Аккаунты
                </button>
                <button class="market-category" type="button" role="tab" aria-selected="false" data-category="channels">
                    Каналы
                </button>
                <button class="market-category" type="button" role="tab" aria-selected="false" data-category="services">
                    Услуги
                </button>
                <button class="market-category" type="button" role="tab" aria-selected="false" data-category="boosting">
                    Накрутка
                </button>
            </div>
            <div class="category-description" id="categoryDescription">
                Проверенные Telegram-аккаунты с быстрой выдачей после покупки.
            </div>
        </section>
        ...
        <section class="section">
            ...
            <div class="loader" id="loader"><div class="spinner"></div><span class="loader-label">Загружаю…</span></div>
    ...
    <div class="page" id="pageProfile">
        <div class="profile-hero">
            ...
            <span class="profile-watermark"><span class="dot"></span>Маркетплейс</span>
    ...
    <div class="modal hidden pdetail-sheet" id="purchaseDetailModal">
        ...
        <div class="modal-sheet">
            ...
            <div class="pdetail-actions" id="pdetailActions">
                <button class="pur-btn confirm" id="pdetailConfirmBtn">
    ...
    <div class="page" id="pageSell">
        <div class="topup-hero">
            ...
            <div class="topup-title">Разместить товар</div>
            <div class="topup-sub">Аккаунты, каналы, услуги, подписки и API — в подходящей категории</div>
        </div>

        <div class="sell-form-card generic-sell-card" id="genericSellCard">
            <h3>Новое объявление</h3>
            <div class="sell-mode-switch" role="tablist" aria-label="Тип объявления">
                <button class="sell-mode-switch-btn active" type="button" id="genericSellModeBtn">Товар</button>
                <button class="sell-mode-switch-btn" type="button" id="accountSellModeBtn">Telegram-аккаунт</button>
            </div>
            <div class="generic-sell-grid">
                <label class="sell-label">Игра<select class="sell-input" id="genericSellGame"><option value="telegram">Telegram</option><option value="chatgpt">ChatGPT</option></select></label>
                <label class="sell-label">Категория<select class="sell-input" id="genericSellCategory"></select></label>
                <label class="sell-label">Подкатегория<select class="sell-input" id="genericSellSubtype"></select></label>
            </div>
            <label class="sell-label">Название</label>
            <input class="sell-input" id="genericSellTitle" maxlength="120" placeholder="Например: ChatGPT Plus на 1 месяц">
            <label class="sell-label">Описание</label>
            <textarea class="sell-textarea" id="genericSellDescription" maxlength="2000" placeholder="Опишите состав товара, срок и условия передачи"></textarea>
            <label class="sell-label">Цена (₽)</label>
            <input class="sell-input" id="genericSellPrice" type="number" min="1" max="500000" placeholder="Например: 990">
            <button class="btn-primary" type="button" id="genericSellPublish">Опубликовать товар</button>
            <div class="sell-hint" id="genericSellError" style="color:#ef4444;display:none;"></div>
        </div>
        ...
        <div class="sell-form-card" id="sellStep1" style="display:none;">
            ...
            <label class="sell-pin-option" for="sellPinned">
                <input id="sellPinned" type="checkbox">
                <span class="sell-pin-check" aria-hidden="true">✓</span>
                <span class="sell-pin-copy">
                    <strong>📌 Закрепить объявление</strong>
                    <small>Лот будет выше обычных и получит выделенную оранжевую карточку.</small>
                    <small>С баланса спишется 10% от цены лота.</small>
                    <small class="sell-pin-fee" id="sellPinFee">Стоимость закрепления: 0 ₽</small>
                </span>
            </label>
    ...
    <div class="side-menu" id="sideMenu" aria-hidden="true">
        ...
        <aside class="side-menu-panel" role="dialog" aria-label="Меню">
            <div class="side-menu-head">
                ...
                <div class="sm-sub">Маркетплейс</div>
            </div>
            <div class="side-menu-list">
                ...
                <button class="side-menu-item" id="sideMenuSettings" type="button">
                    <span class="sm-label">Настройки</span>
                </button>
    ...
    <div class="modal hidden" id="filtersModal">
        ...
        <div class="modal-sheet">
            ...
            <div class="filter-modal-section account-filter-block">
            ...
            <div class="filter-modal-section category-filter-block hidden" id="categoryFilterFields">
                <div class="filter-modal-label" id="categoryFilterLabel">Параметры категории</div>
                <div class="filter-grid cols-2" id="categoryFilterGrid"></div>
                <div class="category-filter-hint" id="categoryFilterHint"></div>
            </div>
    ...
    <div class="modal hidden" id="itemModal">
        ...
        <div class="modal-sheet">
            ...
            <ul class="item-features" id="itemFeatures">
                <li data-feature="pinned" class="hidden">📌 Закреплённое объявление</li>
            ...
            <button class="btn-confirm-buy hidden" id="marketBuyBtn" type="button">
                Купить товар
            </button>
    ...
    <script>
        (function () {
            ...
            const dom = {
                ...
                genericSellGame: $('genericSellGame'),
                genericSellCategory: $('genericSellCategory'),
                genericSellSubtype: $('genericSellSubtype'),
                genericSellTitle: $('genericSellTitle'),
                genericSellDescription: $('genericSellDescription'),
                genericSellPrice: $('genericSellPrice'),
                genericSellPublish: $('genericSellPublish'),
                genericSellError: $('genericSellError'),
                genericSellModeBtn: $('genericSellModeBtn'),
                accountSellModeBtn: $('accountSellModeBtn'),
                ...
                categoryFilterGrid: $('categoryFilterGrid'),
                categoryFilterHint: $('categoryFilterHint'),
            ...
            function renderPurchases() {
                ...
                for (const p of purchasesState.items) {
                    ...
                    if (p.kind === 'market_order') {
                        card.innerHTML = `
                            <div class="pcard-top"><div class="pcard-title">${escapeHtml(p.title || 'Товар')}</div>
                            <span class="pcard-origin">${escapeHtml(GAME_LABELS[p.game] || p.game)} · ${escapeHtml(p.category_label || p.category || '')}</span></div>
                            <div class="pcard-price">${formatRub(p.amount)}<span class="rub">₽</span></div>
                            <div class="pcard-meta"><span class="pcard-meta-badge ${p.status === 'confirmed' ? 'session' : 'paid'}">${p.status === 'confirmed' ? 'Получение подтверждено' : 'Ожидает подтверждения'}</span></div>
                            ${p.status === 'confirmed' ? '' : '<button type="button" class="pur-btn confirm purchase-confirm-inline">Подтвердить получение</button>'}`;
                        const confirm = card.querySelector('.purchase-confirm-inline');
                        if (confirm) confirm.addEventListener('click', (e) => { e.stopPropagation(); confirmMarketOrder(p.order_id, confirm); });
                    ...
                    metaBadges.push(p.status === 'confirmed'
                        ? '<span class="pcard-meta-badge session">✅ Получение подтверждено</span>'
                        : '<span class="pcard-meta-badge paid">⏳ Ожидает подтверждения</span>');
            ...
            async function confirmPurchaseReceipt(purchaseId) {
                const item = purchasesState.items.find((x) => Number(x.id) === Number(purchaseId));
                if (!item || item.status === 'confirmed') return;
                const confirmed = window.confirm(
                    'Подтвердить получение товара? После подтверждения деньги перейдут продавцу в холд.'
                );
                if (!confirmed) return;
                const btn = document.getElementById('pdetailConfirmBtn');
                if (btn) {
                    btn.disabled = true;
                ...
                try {
                    const resp = await fetch(`/api/purchases/${purchaseId}/confirm`, {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            ...(state.initData ? { 'X-Init-Data': state.initData } : {}),
                        },
                    });
                    const data = await resp.json();
                    if (!data.ok) {
                        showToast(data.detail || 'Не удалось подтвердить получение', 'error');
                        return;
                    }
                    item.status = 'confirmed';
                    item.can_confirm = false;
                    showToast('✅ Получение подтверждено. Средства переведены продавцу в холд.', 'success');
                    closePurchaseDetail();
                    renderPurchases();
                } catch (e) {
                    showToast('Ошибка сети при подтверждении', 'error');
                } finally {
                    if (btn) {
                        btn.disabled = true;
                        btn.classList.remove('loading');
                        btn.textContent = '✅ Получение подтверждено';
                    }
                }
            }

            async function confirmMarketOrder(orderId, button) {
                if (button) { button.disabled = true; button.textContent = 'Подтверждаем…'; }
                try {
                    const resp = await fetch(`/api/market/orders/${orderId}/confirm`, {
                        method: 'POST',
                        headers: state.initData ? { 'X-Init-Data': state.initData } : {},
                    });
                    const data = await resp.json();
                    if (!data.ok) throw new Error(data.detail || data.error || 'Ошибка подтверждения');
                    showToast('Получение подтверждено. Средства в холде продавца.', 'success');
                    loadPurchases();
                } catch (e) {
                    showToast(e.message, 'error');
                    if (button) { button.disabled = false; button.textContent = 'Подтвердить получение'; }
                }
            }
            ...
            function openPurchaseDetail(p) {
                ...
                const confirmBtn = document.getElementById('pdetailConfirmBtn');

                if (codeBtn)    codeBtn.disabled    = !hasSession;
                if (sessionBtn) sessionBtn.disabled = !hasSession;
                if (jsonBtn)    jsonBtn.disabled     = !hasSession;
                if (confirmBtn) {
                    confirmBtn.disabled = p.status === 'confirmed' || p.can_confirm === false;
                    confirmBtn.textContent = p.status === 'confirmed'
                ...
                rebind(document.getElementById('pdetailConfirmBtn'), () => {
                    confirmPurchaseReceipt(p.id);
                });
  ...
  loadProfileListings();
            ...
            function renderChats(animate) {
                ...
                for (const c of chatsState.items) {
                    ...
                    // sender_id === 0 — это служебное сообщение бота.
                    ...
                    let _prefix = '';
                    if (_lastSid === 0) _prefix = 'Поддержка: ';
            ...
            function renderChatMessages(animate) {
                ...
                for (const m of msgs) {
                    // sender_id === 0 — это служебное сообщение. Рисуем отдельным
            ...
            // Рендер «служебного» сообщения от бота.
            ...
            function renderBotMessage(m) {
                ...
                const headHtml = (
                    '<div class="chat-bubble-bot-head">' +
                        ...
                        '</div>' +
                        '<div>' +
                            '<div class="chat-bubble-bot-name">Поддержка' +
                    ...
                    actionsHtml = '<div class="chat-bubble-bot-actions">' +
                        actions.map((a) => {
                            ...
                            const extraClass = (a.action === 'open_review')
                                ? ' secondary'
                                : (a.action === 'confirm_receipt' ? ' confirm' : '');
            ...
            function handleBotAction(action, param) {
                ...
                if (action === 'confirm_receipt') {
                    const pid = Number(param || 0);
                    if (pid > 0) {
                        closeModal('chatModal');
                        const item = (purchasesState.items || []).find((x) => Number(x.id) === pid);
                        if (item) {
                            openPurchaseDetail(item);
                            confirmPurchaseReceipt(pid);
                        } else {
                            switchPage('pagePurchases');
                            loadPurchases().then(() => {
                                const fresh = purchasesState.items.find((x) => Number(x.id) === pid);
                                if (fresh) {
                                    openPurchaseDetail(fresh);
                                    confirmPurchaseReceipt(pid);
                                }
                            });
                        }
            ...
            */
            ...
            function sellReadForm() {
                ...
                const isPinned = !!document.getElementById('sellPinned')?.checked;
                return { title, description, price, regMonth, regYear, isPinned };
            }
            ...
            function updateSellSummary() {
                const { title, description, price, regMonth, regYear, isPinned } = sellReadForm();
                ...
                const pinFee = Math.round((price || 0) * 10) / 100;
                const origin = sellState.origin || '—';
                const pinFeeEl = document.getElementById('sellPinFee');
                if (pinFeeEl) {
                    pinFeeEl.textContent = 'Стоимость закрепления: ' + formatMoney(pinFee) + ' ₽';
                }
                ...
                sumEl.innerHTML =
                    ...
                    (isPinned
                        ? '<br>📌 Закрепление: <b>' + formatMoney(pinFee) + ' ₽ спишется сейчас</b>'
                        : '');
            ...
            function onListingPublished(data) {
                ...
                const pinMessage = data.is_pinned
                    ? ' 📌 Закреплено за ' + formatMoney(data.pin_fee) + ' ₽.'
                    : '';
                showToast(
                    '✅ Объявление #' + data.listing_id + ' опубликовано! ' +
                    'Страна: ' + (data.country || '—') + '. ' +
                    'Вам поступит: ' + data.net + '₽.' + pinMessage,
                    'success'
                );
                resetSellForm();
            ...
            function resetSellForm() {
                ...
                const pinnedInput = document.getElementById('sellPinned');
                if (pinnedInput) pinnedInput.checked = false;
                const pinFeeEl = document.getElementById('sellPinFee');
                if (pinFeeEl) pinFeeEl.textContent = 'Стоимость закрепления: 0 ₽';
            ...
            async function loadMyListings() {
                ...
                try {
                    ...
                    wrap.innerHTML = items.map((it) => {
                        ...
                        return (
                            '<div class="sell-my-item' + (it.is_pinned ? ' pinned' : '') + '" data-listing-id="' + it.id + '" data-account-id="' + (it.account_id || '') + '">' +
                                '<div class="mi-row">' +
                                    '<div class="mi-title">' + escapeHtml(it.title) +
                                        (it.is_pinned ? ' <span class="pin-inline-badge">📌 Закреплено</span>' : '') + '</div>' +
            ...
            function populateGenericSellCategories() {
                if (!dom.genericSellGame || !dom.genericSellCategory || !dom.genericSellSubtype) return;
                const game = dom.genericSellGame.value || 'telegram';
                const cats = GAME_CATEGORIES[game] || GAME_CATEGORIES.telegram;
                dom.genericSellCategory.innerHTML = cats.map((c) => `<option value="${c}">${escapeHtml(MARKET_CATEGORIES[c]?.title || c)}</option>`).join('');
                const cat = dom.genericSellCategory.value;
                const info = MARKET_CATEGORIES[cat] || {};
                const filters = game === 'chatgpt' && cat === 'accounts' ? ['all', 'Plus', 'Team', 'Enterprise'] : (info.filters || ['all']);
                dom.genericSellSubtype.innerHTML = filters.map((x) => `<option value="${x}">${escapeHtml(x === 'all' ? 'Без уточнения' : x)}</option>`).join('');
            }

            async function publishGenericItem() {
                const game = dom.genericSellGame?.value || 'telegram';
                const category = dom.genericSellCategory?.value || '';
                const subtype = dom.genericSellSubtype?.value || 'all';
                const title = (dom.genericSellTitle?.value || '').trim();
                const description = (dom.genericSellDescription?.value || '').trim();
                const price = Number(dom.genericSellPrice?.value || 0);
                if (!title || !description || !price || price < 1) {
                    if (dom.genericSellError) { dom.genericSellError.textContent = 'Заполните название, описание и цену'; dom.genericSellError.style.display = ''; }
                    return;
                }
                if (dom.genericSellPublish) { dom.genericSellPublish.disabled = true; dom.genericSellPublish.textContent = 'Публикуем…'; }
                try {
                    const r = await api('/api/market/items', {
                        method: 'POST',
                        body: JSON.stringify({ game, category, subtype: subtype === 'all' ? '' : subtype, title, description, price }),
                    });
                    if (!r.ok) throw new Error(r.data?.detail || r.data?.error || 'Не удалось опубликовать');
                    showToast('Товар опубликован', 'success');
                    if (dom.genericSellTitle) dom.genericSellTitle.value = '';
                    if (dom.genericSellDescription) dom.genericSellDescription.value = '';
                    if (dom.genericSellPrice) dom.genericSellPrice.value = '';
                    if (state.game === game && state.category === category) loadCatalog();
                } catch (e) {
                    if (dom.genericSellError) { dom.genericSellError.textContent = e.message; dom.genericSellError.style.display = ''; }
                } finally {
                    if (dom.genericSellPublish) { dom.genericSellPublish.disabled = false; dom.genericSellPublish.textContent = 'Опубликовать товар'; }
                }
            }

            function bindGenericSellEvents() {
                if (!dom.genericSellGame) return;
                populateGenericSellCategories();
                dom.genericSellGame.addEventListener('change', populateGenericSellCategories);
                dom.genericSellCategory?.addEventListener('change', populateGenericSellCategories);
                dom.genericSellPublish?.addEventListener('click', publishGenericItem);
                dom.genericSellModeBtn?.addEventListener('click', () => setSellMode('generic'));
                dom.accountSellModeBtn?.addEventListener('click', () => setSellMode('account'));
                setSellMode('generic');
            }

            function setSellMode(mode) {
                const generic = mode !== 'account';
                const genericCard = document.getElementById('genericSellCard');
                const accountStep1 = document.getElementById('sellStep1');
                const accountStep2 = document.getElementById('sellStep2');
                const summary = document.getElementById('sellSummaryCard');
                if (genericCard) genericCard.style.display = generic ? '' : 'none';
                if (accountStep1) accountStep1.style.display = generic ? 'none' : '';
                if (accountStep2) accountStep2.style.display = 'none';
                if (summary) summary.style.display = 'none';
                dom.genericSellModeBtn?.classList.toggle('active', generic);
                dom.accountSellModeBtn?.classList.toggle('active', !generic);
            }
            ...
            function bindSellEvents() {
                ...
                const pinnedInput = document.getElementById('sellPinned');
                if (pinnedInput) {
                    pinnedInput.addEventListener('change', updateSellSummary);
                }
            ...
            const GAME_LABELS = { telegram: 'Telegram', chatgpt: 'ChatGPT' };
            const MARKET_CATEGORIES = {
                accounts: {
                    title: 'Аккаунты',
                    description: 'Проверенные Telegram-аккаунты с быстрой выдачей после покупки.',
                    emptyIcon: '👤',
                },
                channels: {
                    title: 'Каналы',
                    description: 'Каналы для рекламы и развития проектов — раздел готовится к запуску.',
                    emptyIcon: '📣',
                    filterLabel: 'Тип канала',
                    filters: ['all', 'Публичный', 'Приватный', 'С рекламой'],
                    hint: 'Категория работает отдельно от аккаунтов. Публикация каналов появится позже.',
                },
                services: {
                    title: 'Услуги',
                    description: 'Оформление, настройка и сопровождение Telegram-проектов.',
                    emptyIcon: '🛠️',
                    filterLabel: 'Тип услуги',
                    filters: ['all', 'Оформление', 'Настройка', 'Продвижение'],
                    hint: 'Здесь будут услуги исполнителей. Добавление аккаунтов в эту категорию запрещено.',
                },
                boosting: {
                    title: 'Накрутка',
                    description: 'Продвижение публикаций и каналов с понятными условиями и сроками.',
                    emptyIcon: '🚀',
                    filterLabel: 'Площадка',
                    filters: ['all', 'Telegram', 'Instagram', 'TikTok', 'YouTube'],
                    hint: 'Накрутка будет оформляться отдельными заказами, а не объявлениями аккаунтов.',
                },
                subscriptions: {
                    title: 'Подписки', description: 'Подписки и тарифы ChatGPT от проверенных продавцов.',
                    emptyIcon: '•', filterLabel: 'Тариф', filters: ['all', 'Plus', 'Pro', 'Team'],
                    hint: 'Выбирайте тариф и срок действия. Доступ передаётся после подтверждения заказа.',
                },
                api: {
                    title: 'API', description: 'API-доступ и лимиты для проектов на базе ChatGPT.',
                    emptyIcon: '•', filterLabel: 'Тип доступа', filters: ['all', 'Ключ', 'Пакет запросов', 'Лимит'],
                    hint: 'Условия доступа указываются продавцом в описании лота.',
                },
            };

            function categoryInfo() {
                return MARKET_CATEGORIES[state.category] || MARKET_CATEGORIES.accounts;
            }
            ...
            function renderGameTabs() {
                document.querySelectorAll('.game-tab').forEach((btn) => {
                    ...
                    btn.hidden = !!state.gameSearch && !btn.textContent.toLowerCase().includes(state.gameSearch.toLowerCase());
                });
            }

            function renderCategoryChrome() {
                const info = categoryInfo();
                if (dom.marketCategories) {
                    dom.marketCategories.innerHTML = '';
                    (GAME_CATEGORIES[state.game] || GAME_CATEGORIES.telegram).forEach((key) => {
                        const btn = document.createElement('button');
                        btn.className = 'market-category' + (key === state.category ? ' active' : '');
                        btn.type = 'button'; btn.role = 'tab';
                        btn.dataset.category = key;
                        btn.setAttribute('aria-selected', key === state.category ? 'true' : 'false');
                        btn.textContent = MARKET_CATEGORIES[key]?.title || key;
                        btn.addEventListener('click', () => switchMarketCategory(key));
                        dom.marketCategories.appendChild(btn);
                    });
                }
                ...
                if (dom.categoryDescription) dom.categoryDescription.textContent = info.description;
                renderGameTabs();
                const isAccounts = state.game === 'telegram' && state.category === 'accounts';
                document.querySelectorAll('.account-filter-block').forEach((el) => {
                    el.classList.toggle('hidden', !isAccounts);
                });
                if (dom.categoryFilterFields) {
                    dom.categoryFilterFields.classList.toggle('hidden', isAccounts);
                }
                if (dom.filterSummary) {
                    dom.filterSummary.textContent = isAccounts
                        ? 'Все страны · Любое · По умолчанию'
                        : `${info.title} · ${state.categoryFilter === 'all' ? 'Все варианты' : state.categoryFilter}`;
                }
            }

            function renderCategoryFilters() {
                const info = categoryInfo();
                if (!dom.categoryFilterGrid || !dom.categoryFilterLabel) return;
                const chatgptAccounts = state.game === 'chatgpt' && state.category === 'accounts';
                const filters = chatgptAccounts ? ['all', 'Plus', 'Team', 'Enterprise'] : (info.filters || ['all']);
                dom.categoryFilterLabel.textContent = chatgptAccounts ? 'Тип аккаунта' : (info.filterLabel || 'Параметры категории');
                dom.categoryFilterGrid.innerHTML = '';
                filters.forEach((value) => {
                    const btn = document.createElement('button');
                    btn.type = 'button';
                    btn.className = 'filter-chip' + (state.categoryFilter === value ? ' active' : '');
                    btn.dataset.categoryFilter = value;
                    btn.textContent = value === 'all' ? 'Все' : value;
                    btn.addEventListener('click', () => {
                        state.categoryFilter = value;
                        renderCategoryFilters();
                    });
                    dom.categoryFilterGrid.appendChild(btn);
                });
                if (dom.categoryFilterHint) dom.categoryFilterHint.textContent = info.hint || '';
            }
                ...
                if (dom.catalog) {
                    dom.catalog.innerHTML = `
                        <div class="category-empty">
                            <div class="category-empty-icon">${info.emptyIcon}</div>
                            <strong>${escapeHtml(info.title)} скоро появятся</strong>
                            <p>${escapeHtml(info.hint || 'Категория находится в разработке.')}</p>
                        </div>`;
                }
                if (dom.catListCount) dom.catListCount.textContent = '0';
                showEmpty(false);
                showLoader(false);
            }
            ...
            async function switchMarketCategory(category) {
                if (!MARKET_CATEGORIES[category] || state.category === category) return;
                state.category = category;
                resetAccountFilters();
                renderCategoryChrome();
                renderCategoryFilters();
                if (state.game === 'telegram' && category === 'accounts') await loadCategories();
                await loadCatalog();
            }

            async function switchGame(game) {
                if (!GAME_CATEGORIES[game] || state.game === game) return;
                state.game = game;
                state.category = GAME_CATEGORIES[game][0];
                resetAccountFilters();
                renderCategoryChrome();
                renderCategoryFilters();
                await loadCatalog();
            ...
            async function loadCatalog() {
                showLoader(true);
                const genericFeed = state.game !== 'telegram' || state.category !== 'accounts';
                if (genericFeed) {
                    const params = new URLSearchParams({
                        game: state.game, category: state.category, limit: '100',
                    });
                    if (state.categoryFilter !== 'all') params.set('subtype', state.categoryFilter);
                    if (state.gameSearch) params.set('search', state.gameSearch);
                    try {
                        const r = await api('/api/market/items?' + params.toString());
                        state.catalog = r.ok ? (r.data.items || []) : [];
                        state.catalogSig = state.catalog.map((it) => `${it.id}:${it.price}`).join('|');
                        renderCatalog();
                        updateFilterSummary();
                    } catch (e) {
                        renderLoadFailure('Не удалось загрузить товары.');
                    } finally {
                        showLoader(false);
                    }
                    return;
                ...
                try {
                    ...
                    state.catalogSig = state.catalog.map((it) => `${it.id}:${it.price}:${it.country}:${it.is_pinned ? 1 : 0}`).join('|');
                    renderCatalog();
                    updateFilterSummary();
                } catch (error) {
            ...
            function updateFilterSummary() {
                ...
                if (!(state.game === 'telegram' && state.category === 'accounts')) {
                    const info = categoryInfo();
                    dom.filterSummary.textContent = `${info.title} · ${state.categoryFilter === 'all' ? 'Все варианты' : state.categoryFilter}`;
                    if (dom.filterBadge) {
                        dom.filterBadge.hidden = state.categoryFilter === 'all';
                        dom.filterBadge.textContent = state.categoryFilter === 'all' ? '' : '1';
                    }
                    return;
                }
            ...
            function renderMarketCatalog() {
                ...
                state.catalog.forEach((it) => {
                    ...
                    const sellerName = fullName || (it.seller_username ? '@' + it.seller_username : 'Продавец');
                    const initial = (sellerName.replace('@', '').charAt(0) || '?').toUpperCase();
                    const avatar = it.seller_photo_url
                        ? `<img src="${escapeHtml(it.seller_photo_url)}" alt="" loading="lazy">`
                        : escapeHtml(initial);
                    card.innerHTML = `
                        <div class="market-card-kicker">${escapeHtml(GAME_LABELS[it.game] || it.game)} · ${escapeHtml(it.category_label || it.category)}</div>
                        <div class="card-title-block"><div class="card-lot-title">${escapeHtml(it.title)}</div></div>
                        <div class="market-card-desc">${escapeHtml(it.description || 'Описание появится после открытия лота.')}</div>
                        <div class="card-price">${formatPrice(it.price)}<span class="rub">₽</span></div>
                        <div class="card-seller-line">
                            <div class="card-seller-avatar" data-seller-id="${Number(it.seller_id || 0)}">${avatar}</div>
                            <div class="card-seller-text" data-seller-id="${Number(it.seller_id || 0)}"><span class="card-seller-name">${escapeHtml(sellerName)}</span></div>
                        </div>`;
                    card.addEventListener('click', () => openItem(it));
                    frag.appendChild(card);
                });
                dom.catalog.appendChild(frag);
                hydrateSellerAvatars(dom.catalog);
            }

            async function hydrateSellerAvatars(root) {
                const nodes = [...(root || document).querySelectorAll('[data-seller-id]')]
                    .filter((el) => el.classList.contains('card-seller-avatar'));
                const ids = [...new Set(nodes.map((el) => Number(el.dataset.sellerId)).filter(Boolean))];
                await Promise.allSettled(ids.map(async (id) => {
                    try {
                        const resp = await api(`/api/user/${id}/avatar`);
                        const url = resp.ok && resp.data && resp.data.photo_url;
                        if (!url) return;
                        nodes.filter((el) => Number(el.dataset.sellerId) === id).forEach((el) => {
                            el.innerHTML = `<img src="${escapeHtml(url)}" alt="" loading="lazy">`;
                        });
                    } catch (_) { /* initials remain visible */ }
                }));
            }
            ...
            function renderCatalog() {
                ...
                state.catalog.forEach((it) => {
                    const card = document.createElement('div');
                    card.className = 'card' + (it.is_pinned ? ' pinned' : '');
                    // ====== Полное имя продавца ======
                    ...
                    const premiumBadgeHtml = it.has_premium
                        ...
                        : '';
                    const pinBadgeHtml = it.is_pinned
                        ? `<div class="card-pin-badge" title="Закреплённое объявление">📌 Закреплено</div>`
                        : '';
            ...
            function applyFiltersFromModal() {
                if (state.category !== 'accounts') {
                    const sel = dom.categoryFilterGrid
                        ? dom.categoryFilterGrid.querySelector('.filter-chip.active')
                        : null;
                    state.categoryFilter = sel ? sel.dataset.categoryFilter : 'all';
            ...
            function resetValidateUI() {
                ...
                if (dom.marketBuyBtn) dom.marketBuyBtn.classList.add('hidden');
                if (dom.buyBtn) {
            ...
            function openMarketItem(it) {
                buyState.item = it;
                resetValidateUI();
                if (dom.itemFlag) dom.itemFlag.style.display = 'none';
                if (dom.itemCountry) dom.itemCountry.textContent = it.title || 'Товар';
                if (dom.itemOrigin) dom.itemOrigin.textContent = `${GAME_LABELS[it.game] || it.game} · ${it.category_label || it.category}`;
                if (dom.itemPrice) dom.itemPrice.textContent = formatPrice(Number(it.price) || 0) + ' ₽';
                if (dom.itemDescription) dom.itemDescription.textContent = it.description || 'Описание появится после загрузки.';
                if (dom.validateStatus) dom.validateStatus.classList.add('hidden');
                if (dom.buyBtn) dom.buyBtn.classList.add('hidden');
                if (dom.buyConfirmBtn) dom.buyConfirmBtn.classList.add('hidden');
                if (dom.itemFeatures) dom.itemFeatures.innerHTML = '<li>Товар передаётся продавцом после оплаты</li><li>Подтверждение получения обязательно</li>';
                if (dom.marketBuyBtn) {
                    dom.marketBuyBtn.classList.remove('hidden');
                    dom.marketBuyBtn.disabled = false;
                    dom.marketBuyBtn.textContent = 'Купить товар';
                    dom.marketBuyBtn.onclick = buyMarketItem;
                }
                const seller = (it.seller_full_name || '').trim() || (it.seller_username ? '@' + it.seller_username : 'Продавец');
                if (dom.itemSeller) dom.itemSeller.textContent = seller;
                if (dom.itemRating) dom.itemRating.textContent = 'Проверенный продавец';
                const avatar = document.getElementById('itemSellerAvatar');
                if (avatar) avatar.textContent = (seller.replace('@', '').charAt(0) || '?').toUpperCase();
                openModal('itemModal');
            }

            async function buyMarketItem() {
                const it = buyState.item;
                if (!it || buyState.busy) return;
                if (!window.confirm('Купить этот товар? После получения его нужно будет подтвердить.')) return;
                buyState.busy = true;
                if (dom.marketBuyBtn) { dom.marketBuyBtn.disabled = true; dom.marketBuyBtn.textContent = 'Покупаем…'; }
                try {
                    const r = await api(`/api/market/items/${it.id}/buy`, { method: 'POST', body: '{}' });
                    if (!r.ok) { showToast(translateBuyError(r.data?.error || r.error), 'error'); return; }
                    showToast('Покупка создана. Подтвердите получение в разделе «Мои покупки».', 'success');
                    closeModal('itemModal');
                    refreshBalance({ silent: true });
                    loadCatalog();
                } catch (_) { showToast('Ошибка сети', 'error'); }
                finally { buyState.busy = false; }
            }

            function openItem(it) {
                if (it && it.kind === 'market') {
                    openMarketItem(it);
                    return;
                }
                buyState.item = it;
                ...
                if (featuresRoot) {
                    const pinnedLi = featuresRoot.querySelector('[data-feature="pinned"]');
                    if (pinnedLi) pinnedLi.classList.toggle('hidden', !it.is_pinned);
            ...
            */
            ...
            function renderProfileListingCards(container, items, showStatus) {
                ...
                items.forEach((it) => {
                    ...
                    card.className = 'profile-listing-card' + (it.is_pinned ? ' pinned' : '');
                    ...
                    card.innerHTML =
                        '<div class="profile-listing-row">' +
                            '<div class="profile-listing-name">' + escapeHtml(it.title || 'Telegram-аккаунт') +
                                (it.is_pinned ? ' <span class="pin-inline-badge">📌 Закреплено</span>' : '') + '</div>' +
                ...
                try {
                    ...
                    if (data && data.ok) {
                        ...
                        setTimeout(() => {
                            ...
                            // чтобы юзер увидел благодарность от поддержки.
                            if (
            ...
            function bindEvents() {
                document.querySelectorAll('.game-tab').forEach((btn) => {
                    btn.addEventListener('click', () => switchGame(btn.dataset.game).catch(() => showToast('Не удалось открыть игру', 'error')));
                });
                if (dom.gameSearch) {
                    dom.gameSearch.addEventListener('input', () => {
                        state.gameSearch = dom.gameSearch.value.trim();
                        renderGameTabs();
                        if (state.gameSearch) loadCatalog();
                    });
                }
                document.querySelectorAll('.market-category').forEach((btn) => {
                    btn.addEventListener('click', () => {
                        switchMarketCategory(btn.dataset.category).catch(() => {
                            showToast('Не удалось открыть категорию', 'error');
                        });
                    });
                });
                ...
                if (dom.sideMenuSettings) {
                    dom.sideMenuSettings.addEventListener('click', () => {
                        const theme = document.getElementById('themeToggle');
                        openSideMenu();
                        if (theme) {
                            setTimeout(() => theme.focus(), 0);
                            const next = document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark';
                            applyTheme(next);
                            showToast(next === 'dark' ? 'Тёмная тема включена' : 'Светлая тема включена', 'info');
                        }
                    });
                }
            ...
            async function silentRefreshCatalog() {
                if (state.category !== 'accounts') return;
                const params = new URLSearchParams();
                if (state.country && state.country !== 'all') params.set('country', state.country);
                if (state.origin && state.origin !== 'all') params.set('origin', state.origin);
                if (state.priceSort && state.priceSort !== 'default') params.set('sort', state.priceSort);
                ...
                try {
                    ...
                    const sig = freshItems.map((it) => `${it.id}:${it.price}:${it.country}:${it.is_pinned ? 1 : 0}`).join('|');
...
def api_purchases(telegram_id, tg_user):
    ...
    if not _ensure_purchase_confirmation_columns():
        return jsonify({"ok": False, "error": "confirmation_unavailable"}), 503
    session = SessionLocal()
    try:
        ...
        purchase_meta = {
            ...
            for row in session.execute(
                text(
                    "SELECT id, status, confirmed_at FROM purchases "
        ...
        for p in rows:
            ...
            items.append({
                ...
                "status": purchase_meta.get(int(p.id), {}).get(
                    "status", "awaiting_confirmation"
                ),
                "confirmed_at": (
                    purchase_meta.get(int(p.id), {}).get("confirmed_at").isoformat()
                    if purchase_meta.get(int(p.id), {}).get("confirmed_at") else None
                ),
                "can_confirm": purchase_meta.get(int(p.id), {}).get(
                    "status", "awaiting_confirmation"
                ) != "confirmed",
            })
        if _ensure_market_tables():
            generic_rows = session.execute(
                select(MarketOrder, MarketItem)
                .join(MarketItem, MarketItem.id == MarketOrder.item_id)
                .where(MarketOrder.buyer_id == telegram_id)
                .order_by(MarketOrder.created_at.desc())
            ).all()
            for order, item in generic_rows:
                items.append({
                    "id": f"market-{order.id}", "kind": "market_order",
                    "order_id": order.id, "title": item.title,
                    "amount": float(order.amount or 0),
                    "game": item.game, "category": item.category,
                    "category_label": MARKET_CATEGORY_LABELS.get(item.category, item.category),
                    "created_at": order.created_at.isoformat() if order.created_at else None,
                    "status": order.status, "can_confirm": order.status != "confirmed",
                })
...
@require_auth
def api_confirm_purchase(telegram_id, tg_user, purchase_id):
    """Buyer confirms receipt; only then is seller money put into Hold."""
    if not _ensure_purchase_confirmation_columns():
        return jsonify({
            "ok": False,
            "error": "confirmation_unavailable",
            "detail": "Подтверждение получения временно недоступно",
        }), 503
    ...
    try:
        ...
        meta = session.execute(
            text(
                "SELECT status, confirmed_at FROM purchases "
        ...
        if not account or not account.seller_id:
            return jsonify({
                "ok": False,
                "error": "seller_missing",
                "detail": "Продавец этого товара не найден",
            }), 409
        ...
        if not seller:
            return jsonify({
                "ok": False,
                "error": "seller_missing",
                "detail": "Профиль продавца не найден",
            }), 409
        ...
        hold = session.execute(
            select(Hold).where(Hold.purchase_id == purchase.id)
            .with_for_update()
        ).scalar_one_or_none()
        price = float(purchase.amount or 0.0)
        commission = round(price * COMMISSION_PERCENT / 100.0, 2)
        ...
        session.execute(
            text(
                ...
                "confirmed_at = :confirmed_at WHERE id = :purchase_id"
            ),
            {"confirmed_at": confirmed_at, "purchase_id": int(purchase.id)},
        )
        session.commit()
        return jsonify({
            "ok": True,
            ...
            "status": "confirmed",
            "hold_status": "hold",
            "seller_hold_balance": round(float(seller.hold_balance or 0.0), 2),
            "release_at": hold.release_at.isoformat() if hold.release_at else None,
        })
    except Exception as e:
        session.rollback()
        return jsonify({
...
def api_catalog():
    ...
    try:
        ...
        # Закреплённые объявления всегда выше обычных, внутри каждой группы
        # сохраняется выбранная пользователем сортировка.
        order_columns = []
        if pin_schema_ready:
            latest_pin = (
                select(literal_column("COALESCE(listings.is_pinned, FALSE)"))
                .select_from(Listing.__table__)
                .where(
            ...
            order_columns.append(latest_pin.desc())

        # Сортировка по цене
        if sort == "asc":
            order_columns.extend([Account.price.asc(), Account.created_at.desc()])
        elif sort == "desc":
            order_columns.extend([Account.price.desc(), Account.created_at.desc()])
        elif sort == "new":
            order_columns.append(Account.created_at.desc())
        else:
            # по умолчанию — сначала новые
            order_columns.append(Account.created_at.desc())

        q = q.order_by(*order_columns)
        ...
        if account_ids:
            ...
            if pin_schema_ready:
                listing_query = listing_query.add_columns(
                    literal_column(
                        "COALESCE(listings.is_pinned, FALSE)"
                    ).label("listing_is_pinned"),
                    literal_column(
                ...
                listing_rows = session.execute(listing_query).all()
            else:
                listing_rows = [
                    (item, False, 0.0)
                    for item in session.execute(listing_query).scalars().all()
                ]
            for L, is_pinned, pin_fee in listing_rows:
                ...
                if L.account_id not in listings_by_account:
                    ...
                    pinned_by_account[L.account_id] = bool(is_pinned)
                    pin_fee_by_account[L.account_id] = float(pin_fee or 0.0)
        ...
        for a in rows:
            ...
            # Never block the catalog on Telegram Bot API.  A cached photo
            # is used when available; otherwise the UI renders the initials
            # immediately and the profile view can fetch the photo on demand.
            seller_photo_url = (
                _get_cached_telegram_photo_url(int(a.seller_id))
                if a.seller_id else None
            )
            items.append({
                "id": a.id,
                "country": a.country,
                ...
                "is_pinned": bool(pinned_by_account.get(a.id, False)),
                "pin_fee": float(pin_fee_by_account.get(a.id, 0.0)),
            })
        return jsonify({"ok": True, "items": items, "count": len(items)})
    finally:
        session.close()
...
def api_buy(telegram_id, tg_user):
    ...
    проданным, создаём Purchase. Продавец получает деньги только после
    подтверждения получения покупателем — тогда создаётся Hold.
    ...
    try:
        ...
        # Покупатель оплатил лот, но деньги продавцу пока не переводим.
        # После явного подтверждения получения отдельный endpoint создаст
        # Hold и увеличит hold_balance продавца.
        session.execute(
            text(
                "UPDATE purchases SET status = 'awaiting_confirmation' "
                "WHERE id = :purchase_id"
            ),
            {"purchase_id": int(purchase.id)},
        )
        ...
        # из репозитория и именем «Поддержка», а не как сообщение от
        ...
        if account.seller_id and int(account.seller_id) != int(telegram_id):
            try:
                ...
                # под пузырьком. sender_id = BOT_SENDER_ID — это служебный бот.
                ...
                sys_text = (
                    ...
                    f"💰 Сумма: {price:.0f}₽\n"
                    f"💳 Оплата: с баланса\n"
                    f"🕓 После подтверждения: холд продавца на {HOLD_PERIOD_HOURS} ч\n\n"
                    f"Откройте «Мои покупки», чтобы получить данные аккаунта. "
                    f"Если что-то не так — откройте спор. "
                    f"Сначала получите товар и подтвердите получение — после этого "
                    f"средства перейдут продавцу в холд.\n\n"
                    f"Вам доступен отзыв (1–5★) — {REVIEW_AUTO_POST_DAYS} дней.\n\n"
                    f"[[BTN:confirm_receipt:{int(purchase.id)}|✅ Подтвердить получение]]\n"
...
def api_market_items():
    """Fast generic marketplace feed for Telegram/ChatGPT products."""
    if not _ensure_market_tables():
        return jsonify({"ok": False, "error": "market_unavailable"}), 503
    game = (request.args.get("game") or "telegram").strip().lower()
    category = (request.args.get("category") or "accounts").strip().lower()
    subtype = (request.args.get("subtype") or "").strip()
    search = (request.args.get("search") or "").strip()[:80]
    if game not in MARKET_GAME_CATEGORIES or category not in MARKET_GAME_CATEGORIES[game]:
    ...
    session = SessionLocal()
    try:
        q = select(MarketItem, User).join(
            User, User.telegram_id == MarketItem.seller_id, isouter=True
        ).where(
            MarketItem.game == game,
            MarketItem.category == category,
            MarketItem.status == "active",
        )
        if subtype and subtype != "all":
            q = q.where(MarketItem.subtype == subtype)
        if search:
            needle = f"%{search}%"
            q = q.where(
                (MarketItem.title.ilike(needle)) |
                (MarketItem.description.ilike(needle))
            )
        rows = session.execute(q.order_by(MarketItem.created_at.desc()).limit(limit)).all()
        items = []
        for item, seller in rows:
            photo = _get_cached_telegram_photo_url(int(item.seller_id)) if item.seller_id else None
            items.append({
                "id": item.id, "kind": "market", "game": item.game,
                "category": item.category, "category_label": MARKET_CATEGORY_LABELS.get(item.category, item.category),
                "subtype": item.subtype, "title": item.title,
                "description": item.description or "", "price": float(item.price or 0),
...
@require_auth
def api_market_item_create(telegram_id, tg_user):
    """Creates a generic listing; account listings keep their existing flow."""
    if not _ensure_market_tables():
        return jsonify({"ok": False, "error": "market_unavailable"}), 503
    payload = request.get_json(silent=True) or {}
    game = (payload.get("game") or "telegram").strip().lower()
    category = (payload.get("category") or "").strip().lower()
    subtype = (payload.get("subtype") or "").strip()[:80] or None
    title = (payload.get("title") or "").strip()
    description = (payload.get("description") or "").strip()
    try:
        price = float(payload.get("price") or 0)
    ...
    if game not in MARKET_GAME_CATEGORIES or category not in MARKET_GAME_CATEGORIES[game]:
        return jsonify({"ok": False, "error": "bad_category"}), 400
    if game == "telegram" and category == "accounts":
        return jsonify({"ok": False, "error": "accounts_use_telegram_flow"}), 400
    if not title or len(title) > 120 or len(description) > 2000:
        return jsonify({"ok": False, "error": "bad_text"}), 400
    if price < 1 or price > 500000:
        return jsonify({"ok": False, "error": "bad_price"}), 400
    session = SessionLocal()
    try:
        item = MarketItem(
            seller_id=telegram_id, game=game, category=category, subtype=subtype,
            title=title, description=description, price=price, status="active",
        )
        session.add(item)
        session.commit()
        return jsonify({"ok": True, "id": item.id, "item_id": item.id, "price": price})
    except Exception as e:
        session.rollback()
        return jsonify({"ok": False, "error": "server_error", "detail": str(e)[:200]}), 500
    finally:
...
@app.route("/api/market/items/<int:item_id>/buy", methods=["POST"])
@require_auth
def api_market_item_buy(telegram_id, tg_user, item_id):
    """Buys a generic item; seller receives Hold only after receipt confirmation."""
    if not _ensure_market_tables():
        return jsonify({"ok": False, "error": "market_unavailable"}), 503
    session = SessionLocal()
    try:
        item = session.execute(
            select(MarketItem).where(MarketItem.id == item_id).with_for_update()
        ).scalar_one_or_none()
        if not item or item.status != "active":
            return jsonify({"ok": False, "error": "not_found"}), 404
        if int(item.seller_id) == int(telegram_id):
            return jsonify({"ok": False, "error": "self_buy"}), 403
        buyer = session.execute(
            select(User).where(User.telegram_id == telegram_id).with_for_update()
        ).scalar_one_or_none()
        if not buyer or float(buyer.balance or 0) < float(item.price or 0):
            return jsonify({"ok": False, "error": "low_balance"}), 402
        buyer.balance = float(buyer.balance or 0) - float(item.price or 0)
        buyer.total_spent = float(buyer.total_spent or 0) + float(item.price or 0)
        item.status = "sold"
        order = MarketOrder(
            buyer_id=telegram_id, item_id=item.id, amount=float(item.price or 0),
            status="awaiting_confirmation",
        )
        session.add(order)
        session.commit()
        return jsonify({"ok": True, "order_id": order.id, "balance": buyer.balance, "status": order.status})
    except Exception as e:
        session.rollback()
        return jsonify({"ok": False, "error": "server_error", "detail": str(e)[:200]}), 500
    finally:
...
@app.route("/api/market/orders/<int:order_id>/confirm", methods=["POST"])
@require_auth
def api_market_order_confirm(telegram_id, tg_user, order_id):
    if not _ensure_market_tables():
        return jsonify({"ok": False, "error": "market_unavailable"}), 503
    session = SessionLocal()
    try:
        order = session.execute(
            select(MarketOrder).where(
                MarketOrder.id == order_id, MarketOrder.buyer_id == telegram_id
            ).with_for_update()
        ).scalar_one_or_none()
        if not order:
        ...
        item = session.get(MarketItem, order.item_id)
        seller = session.execute(
            select(User).where(User.telegram_id == item.seller_id).with_for_update()
        ).scalar_one_or_none() if item else None
        if not item or not seller:
            return jsonify({"ok": False, "error": "seller_missing"}), 409
        commission = round(float(order.amount or 0) * COMMISSION_PERCENT / 100.0, 2)
        net = round(float(order.amount or 0) - commission, 2)
        hold = session.execute(
            select(MarketHold).where(MarketHold.order_id == order.id).with_for_update()
        ).scalar_one_or_none()
        if not hold:
            hold = MarketHold(
                seller_id=item.seller_id, item_id=item.id, order_id=order.id,
                gross_amount=float(order.amount), commission=commission, net_amount=net,
                status="hold", release_at=datetime.utcnow() + timedelta(hours=HOLD_PERIOD_HOURS),
            )
            session.add(hold)
            seller.hold_balance = float(seller.hold_balance or 0) + net
            seller.total_earned = float(seller.total_earned or 0) + net
        order.status = "confirmed"
        order.confirmed_at = datetime.utcnow()
        session.commit()
        return jsonify({"ok": True, "status": "confirmed", "release_at": hold.release_at.isoformat()})
    except Exception as e:
        session.rollback()
        return jsonify({"ok": False, "error": "server_error", "detail": str(e)[:200]}), 500
    finally:
...
def api_user_public(user_telegram_id, telegram_id, tg_user):
    ...
    try:
        ...
        if pin_schema_ready:
            public_query = public_query.add_columns(
                literal_column(
                    "COALESCE(listings.is_pinned, FALSE)"
                ).label("listing_is_pinned"),
                literal_column(
            ...
            public_rows = session.execute(public_query).all()
        else:
            public_rows = [
                (listing, account, False, 0.0)
                for listing, account in session.execute(public_query).all()
            ]
        ...
        for listing, account, is_pinned, pin_fee in public_rows:
            reg_parts = []
...
# ===== ХЕЛПЕР: ВСТАВИТЬ СООБЩЕНИЕ ОТ БОТА В ЧАТ =====
...
# лица поддержки. Атомарность обеспечивается условием status='hold'
