...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* ====== Бейджи метаданных ======
           Три тега в один ряд: верифицирован, дата регистрации (если есть),
           страна (повторно). Все в одной строке, с wrap если не хватает
           места. По просьбе — дата регистрации как отдельная плашка,
        ...
        /* ====== Описание аккаунта в детальной модалке — редизайн ======
           ...
           подчёркнутый заголовок, аккуратный разделитель. Контент,
            ...
            */
            ...
            function renderCatalog() {
                ...
                state.catalog.forEach((it) => {
                    ...
                    // Страна уже видна шапкой карточки — дублировать нет смысла.
            ...
            function openItem(it) {
                ...
                // ====== Описание аккаунта в модалке — редизайн ======
                // Раньше тут была одна строка plain text. Теперь — полноценный
                // hero-блок с заголовком «О аккаунте», основным текстом и
                // структурированной выжимкой ключевых плюсов аккаунта. Если
                // продавец не оставил своего описания — собираем богатый
                // дефолт из country/origin/verified/reg/premium.
                if (dom.itemDescription) {
                    ...
                    const rows = [];
                    if (country || origin) {
                        const where = [country, origin].filter(Boolean).join(' · ');
                        rows.push(`<div class="desc-row"><span class="desc-emoji">🌍</span><span class="desc-text">Страна выпуска: <strong>${escapeHtml(where)}</strong></span></div>`);
                    }
                    rows.push(`<div class="desc-row"><span class="desc-emoji">${verified ? '✅' : '⚠️'}</span><span class="desc-text">${verified ? 'Сессия <strong>прошла проверку</strong>, вход без сброса' : 'Сессия не верифицирована — уточняйте у продавца'}</span></div>`);
                    if (premium) {
                        rows.push(`<div class="desc-row"><span class="desc-emoji">⭐</span><span class="desc-text">На аккаунте активен <strong>Telegram Premium</strong></span></div>`);
                    }
                    if (sessionAgo) {
                        rows.push(`<div class="desc-row"><span class="desc-emoji">📅</span><span class="desc-text">Регистрация аккаунта: <strong>${escapeHtml(sessionAgo)}</strong></span></div>`);
                    }
                    rows.push(`<div class="desc-row"><span class="desc-emoji">⚡</span><span class="desc-text">Выдача <strong>сразу после оплаты</strong>, логин и пароль приходят в чат</span></div>`);
                    rows.push(`<div class="desc-row"><span class="desc-emoji">🔒</span><span class="desc-text">Без действующих банов и ограничений на момент продажи</span></div>`);

                    const leadText = rawDesc
                        ? escapeHtml(rawDesc)
                        : (verified
                            ? `Аккаунт ${country ? '<strong>' + escapeHtml(country) + '</strong> ' : ''}прошёл полную проверку. Сессия валидна, готова к выдаче сразу после оплаты.`
                            : `Аккаунт ${country ? '<strong>' + escapeHtml(country) + '</strong> ' : ''}готов к выдаче. Рекомендуем уточнить детали у продавца перед покупкой.`);

                    dom.itemDescription.innerHTML =
                        `<div class="item-description-title"><span class="item-description-icon">📋</span>О АККАУНТЕ</div>` +
                        `<div class="item-description-body"><p>${leadText}</p></div>` +
