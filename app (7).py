...
def _validate_session_string(session_str: str) -> dict:
    ...
    try:
        return asyncio.run(_validate_session_string_async(session_str))
    except Exception as e:
...
def api_sell_account_phone_start(telegram_id, tg_user):
    ...
    try:
        result = asyncio.run(_send_code_async(phone))
    except Exception as e:
    ...
    if prev and prev.get("client"):
        try:
            asyncio.run(prev["client"].disconnect())
        except Exception:
            pass
...
def api_sell_account_phone_verify(telegram_id, tg_user):
    ...
    try:
        ...
        try:
            me_phone = asyncio.run(_get_phone())
        except Exception:
            me_phone = phone
...
def api_sell_account_phone_cancel(telegram_id, tg_user):
    ...
    if pending and pending.get("client"):
        try:
            asyncio.run(pending["client"].disconnect())
        except Exception:
            pass
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* Активный пункт нижней навигации — мягкая пульсация */
        .nav-btn.active { animation: floaty 2.4s ease-in-out infinite; }
        ...
        .nav-btn:active, .filter-btn:active, .balance-pill:active,
        .action-item:active, .topup-method:active {
...
<body>
    ...
    <div class="page" id="pageSell">
        <div class="topup-hero">
            <button class="profile-back" id="sellBack" aria-label="Назад">←</button>
        ...
        <div class="sell-info">
            ...
            <div class="sell-info-row">1️⃣ Укажите <b>название</b> объявления</div>
            <div class="sell-info-row">2️⃣ Напишите <b>описание</b></div>
            <div class="sell-info-row">3️⃣ Поставьте свою <b>цену</b></div>
            <div class="sell-info-row">4️⃣ Загрузите аккаунт (<b>.session</b> или вход <b>по коду</b>)</div>
            <div class="sell-info-row">5️⃣ Объявление появится в каталоге</div>
        ...
        <div class="sell-form-card">
            <h3><span class="sell-step-num">1</span>Название объявления</h3>
        ...
        <!-- Шаг 2: описание -->
        <div class="sell-form-card">
            <h3><span class="sell-step-num">2</span>Описание</h3>
            ...
            <div class="sell-hint">До 1000 символов. Или отправьте «-» чтобы оставить пустым.</div>
        </div>

        <!-- Шаг 3: цена -->
        <div class="sell-form-card">
            <h3><span class="sell-step-num">3</span>Цена (₽)</h3>
        ...
        <!-- Шаг 4: происхождение -->
        <div class="sell-form-card">
            <h3><span class="sell-step-num">4</span>Происхождение аккаунта</h3>
        ...
        <!-- Шаг 5: способ загрузки -->
        <div class="sell-form-card">
            <h3><span class="sell-step-num">5</span>Загрузка аккаунта</h3>
            <div class="sell-mode-tabs" id="sellModeTabs">
                <button type="button" class="sell-mode-tab active" data-mode="session">📄 .session файл</button>
                <button type="button" class="sell-mode-tab" data-mode="phone">📱 По коду</button>
            </div>

            <!-- Режим .session -->
            <div id="sellModeSession">
                <label class="sell-file-zone" id="sellFileZone">
                    <span class="file-emoji">📂</span>
                    <span>Нажмите, чтобы выбрать <b>.session</b> файл</span>
                    <span class="file-name" id="sellFileName" style="display:none;"></span>
                    <input type="file" id="sellFileInput" accept=".session,text/plain" style="display:none;">
                </label>
                <div class="sell-hint">Файл получен из Telethon / Pyrogram. Бот проверит сессию автоматически.</div>
            </div>

            <!-- Режим phone+code -->
            <div id="sellModePhone" style="display:none;">
                ...
                <div id="sellCodeWrap" style="display:none; margin-top: 14px;">
                    <label class="sell-label">Код подтверждения</label>
                    <input class="sell-input sell-code-input" id="sellCode" type="text"
                           inputmode="numeric" maxlength="6" placeholder="00000">
                    <div class="sell-hint">Код придёт в Telegram на указанный номер.</div>
                </div>
                ...
                <div id="sell2faWrap" style="display:none; margin-top: 14px;">
                    ...
                    <div class="sell-hint">Аккаунт защищён 2FA. Введите облачный пароль.</div>
                </div>
        ...
        <div class="sell-form-card">
            ...
            <div id="sellSummary" style="font-size: 13.5px; line-height: 1.7; color: var(--gray-700);">
                Заполните все шаги выше — здесь появится сводка.
            </div>
            <button type="button" class="btn-primary" id="sellPublishBtn" style="margin-top: 14px;" disabled>
                <span id="sellPublishBtnText">Опубликовать</span>
            </button>
            <button type="button" class="btn-secondary" id="sellCancelBtn" style="margin-top: 8px;">
                Отменить
            </button>
        </div>
    ...
    <nav class="bottom-nav" id="bottomNav">
        ...
        <button class="nav-btn" data-page="pageSell">
            <span class="nav-emoji">💸</span><span>Продать</span>
        </button>
    ...
    <script>
        (function () {
            ...
            function switchPage(pageId, opts) {
                opts = opts || {};
                const pages = ['pageCatalog', 'pageProfile', 'pagePurchases', 'pageTopup', 'pageSell'];
                pages.forEach((p) => {
            ...
            */
            ...
            function sellValidate({ title, description, price }) {
                ...
                if (sellState.mode === 'session' && !sellState.sessionString) {
                    return { ok: false, msg: 'Загрузите .session файл' };
                }
                if (sellState.mode === 'phone') {
                    const phone = (document.getElementById('sellPhone')?.value || '').trim();
                    if (!phone || !phone.startsWith('+') || phone.length < 8) {
                        return { ok: false, msg: 'Введите номер в формате +79001234567' };
                    }
                    if (!sellState.phoneSent) {
                        return { ok: false, msg: 'Сначала отправьте код на номер' };
                    }
                    if (sellState.needs2fa) {
                        const pwd = (document.getElementById('sell2fa')?.value || '').trim();
                        if (!pwd) return { ok: false, msg: 'Введите пароль 2FA' };
                    } else {
                        const code = (document.getElementById('sellCode')?.value || '').trim();
                        if (!code || code.length < 4) {
                            return { ok: false, msg: 'Введите код подтверждения' };
                        }
                    }
                }
                return { ok: true };
            ...
            function updateSellSummary() {
                ...
                const modeLabel = sellState.mode === 'session' ? '📄 .session файл' : '📱 По коду';

                if (!title && !price) {
                    sumEl.innerHTML = '<i>Заполните все шаги выше — здесь появится сводка.</i>';
                ...
                sumEl.innerHTML =
                    ...
                    '📥 Способ: <b>' + modeLabel + '</b>';

                const v = sellValidate({ title, description, price });
                if (pubBtn) pubBtn.disabled = !v.ok;
            }

            function setSellMode(mode) {
                sellState.mode = mode;
                document.querySelectorAll('#sellModeTabs .sell-mode-tab').forEach((b) => {
                    b.classList.toggle('active', b.dataset.mode === mode);
                });
                document.getElementById('sellModeSession').style.display = mode === 'session' ? '' : 'none';
                document.getElementById('sellModePhone').style.display = mode === 'phone' ? '' : 'none';
                // Сброс phone-состояния при переключении
                if (mode === 'phone') {
                    ...
                    // Отменим pending phone-flow если переключились на .session
            ...
            async function handleSellFile(file) {
                const nameEl = document.getElementById('sellFileName');
                if (!file) {
                    sellState.fileName = null;
                    sellState.sessionString = null;
                    if (nameEl) { nameEl.style.display = 'none'; nameEl.className = 'file-name'; }
                    updateSellSummary();
                    return;
                }
                const isSession = /\.session$/i.test(file.name) ||
                                  file.type === 'text/plain' || file.type === '';
                if (!isSession) {
                    sellState.fileName = file.name;
                    sellState.sessionString = null;
                    if (nameEl) {
                        nameEl.style.display = '';
                        nameEl.textContent = '❌ ' + file.name + ' (нужен .session)';
                ...
                try {
                    ...
                    sellState.fileName = file.name;
                    sellState.sessionString = text;
                    if (nameEl) {
                        nameEl.style.display = '';
                        nameEl.textContent = '✅ ' + file.name + ' (' + (text.length) + ' символов)';
            ...
            async function sellPhoneSendCode() {
                ...
                const { title, description, price } = sellReadForm();
                const v = sellValidate({ title, description, price });
                if (!v.ok && !v.msg.startsWith('Введите номер')) {
                    showToast(v.msg, 'error');
                    return;
                }
                const phone = (document.getElementById('sellPhone')?.value || '').trim();
                if (!phone || !phone.startsWith('+') || phone.length < 8) {
                    showToast('Введите номер в формате +79001234567', 'error');
                    return;
                }
                ...
                try {
                    ...
                    if (!r.ok) {
                        showToast('Не удалось отправить код: ' + (r.data?.detail || r.data?.error || r.error || ''), 'error');
                        return;
                    }
                    ...
                    showToast('Код отправлен на ' + phone, 'success');
                } catch (e) {
                    showToast('Сеть: ' + e.message, 'error');
                } finally {
            ...
            async function publishSellListing() {
                ...
                if (btnText) btnText.innerHTML = '<span class="sell-loader"></span>Публикация…';

                try {
                    let endpoint, body;
                    if (sellState.mode === 'session') {
                        endpoint = '/api/sell_account/session';
                        body = {
                            title, description, price,
                            origin: sellState.origin,
                            session_string: sellState.sessionString,
                        };
                    } else {
                        if (sellState.needs2fa) {
                            endpoint = '/api/sell_account/phone/2fa';
                            const password = (document.getElementById('sell2fa')?.value || '').trim();
                            body = { password };
                        } else {
                            endpoint = '/api/sell_account/phone/verify';
                            const code = (document.getElementById('sellCode')?.value || '').trim();
                            body = { code };
                        }
                    }
                    ...
                    if (!r.ok) {
                        // Спец-кейс: требуется 2FA
                        if (r.data?.error === 'need_2fa') {
                            sellState.needs2fa = true;
                            const twofaWrap = document.getElementById('sell2faWrap');
                            if (twofaWrap) twofaWrap.style.display = '';
                            showToast('Аккаунт защищён 2FA. Введите облачный пароль.', 'error');
                            return;
                        }
                        showToast('Ошибка: ' + (r.data?.detail || r.data?.error || r.error || ''), 'error');
                        return;
                    }

                    showToast(
                        '✅ Объявление #' + r.data.listing_id + ' опубликовано! ' +
                        'Страна: ' + (r.data.country || '—') + '. ' +
                        'Вам поступит: ' + r.data.net + '₽',
                        'success'
                    );
                    resetSellForm();
                    loadMyListings();
            ...
            function bindSellEvents() {
                ...
                // Табы режима
                document.querySelectorAll('#sellModeTabs .sell-mode-tab').forEach((btn) => {
                    btn.addEventListener('click', () => setSellMode(btn.dataset.mode));
                });
                ...
                // Publish / Cancel
                const pubBtn = document.getElementById('sellPublishBtn');
                if (pubBtn) pubBtn.addEventListener('click', publishSellListing);
                const cancelBtn = document.getElementById('sellCancelBtn');
                if (cancelBtn) {
                    cancelBtn.addEventListener('click', () => {
                        // Отменим phone-flow, если он в процессе
                        fetch('/api/sell_account/phone/cancel', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ initData: state.initData || '' }),
...
def api_buy(telegram_id, tg_user):
    ...
    try:
        ...
        # 🚫 FIX ДВОЙНОЙ ПОКУПКИ:
        # Если на этот аккаунт есть активное P2P-объявление (Listing) — нельзя
        # продавать аккаунт напрямую из мини-аппа, иначе тот же аккаунт потом
        # можно будет купить второй раз через бота (там покупка идёт через
        # listing, и бот не знает, что аккаунт уже ушёл из мини-аппа).
        ...
        if active_listings:
            return jsonify({
                "ok": False,
                "error": "has_active_listing",
                "detail": "Этот аккаунт выставлен на P2P-маркетплейсе в боте. "
                          "Купите его там или снимите объявление.",
            }), 409
