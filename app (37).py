...
def build_listing_description(raw_description: str | None, reg_month, reg_year) -> str | None:
    """Склеивает «Регистрация: ...» и пользовательское описание.

    Возвращает:
        - None, если и описание, и дата регистрации пустые (фронт
          покажет дефолтный текст «Аккаунт прошёл проверку…»).
        - строку с префиксом даты регистрации и (через \n\n) исходным
          описанием продавца, если оно есть.

    Формат с \n\n — чтобы на фронте описание нормально переносилось
    между строками, и блок «Регистрация» визуально отделялся от
    свободного текста продавца.
    """
    parts = []
    reg_line = format_reg_info(reg_month, reg_year)
    if reg_line:
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; -webkit-tap-highlight-color: transparent; }
        ...
        /* «Радар» (анимация свечения, пробегающая по карточке при ховере,
           и радиальный градиент сверху) — удалён. Карточка стала проще
           и без лишних визуальных шумов. */
        ...
        /* Реальная авaтарка из Telegram — подгоняется под круглый контейнер */
        ...
        .card-seller-name {
            ...
            /* Полное имя продавца — без обрезки (по просьбе).
               Если строка очень длинная, разрешаем перенос. */
            white-space: normal;
        ...
        /* Реальная аватарка продавца в детальной модалке — в круглом контейнере */
        ...
        .item-seller-name {
            font-size: 14px; font-weight: 700; color: var(--gray-900);
            /* Полное имя продавца — без усечения в модалке тоже */
            white-space: normal;
            word-break: break-word;
            line-height: 1.2;
        }
        ...
        .item-description {
            ...
            /* Префикс «Регистрация: Июль 2021» приходит с \n\n — без
               pre-wrap они склеятся в одну строку, и визуального
               отделения даты регистрации от свободного текста продавца
               не будет. */
        ...
        /* Карточки каталога: лёгкий sheen при наведении
           ...
           Удалён по просьбе — оставляем .card просто .card без декора. */
            ...
            */
            ...
            function renderCatalog() {
                ...
                state.catalog.forEach((it) => {
                    ...
                    card.className = 'card';
                    // ====== Полное имя продавца ======
                    // Берём first_name + last_name вместе — это и есть
                    // «полное имя». Если last_name нет — показываем только
                    // first_name. Если нет ни того, ни другого — username.
                    const sellerFirst = (it.seller_first_name || '').trim();
                    const sellerLast = (it.seller_last_name || '').trim();
                    const fullName = (sellerFirst + (sellerLast ? ' ' + sellerLast : '')).trim();
                    ...
                    // Подпись «юзернейм под именем» — показываем всегда,
                    // когда есть юзернейм и он отличается от полного имени.
                    const handleForSub = it.seller_username ? '@' + it.seller_username : '';
                    const sellerSubHtml = (handleForSub && handleForSub.replace('@','') !== sellerName.replace(/\s.*/, ''))
                        ? `<div class="card-seller-handle">${escapeHtml(handleForSub)}</div>`
                        : '';
                    // ====== Аватарка продавца ======
                    // Если пришла seller_photo_url — рендерим <img>, иначе
                    // фоллбек с первой буквой имени.
                    const avatarHtml = it.seller_photo_url
                        ? `<img src="${escapeHtml(it.seller_photo_url)}" alt="" loading="lazy" onerror="this.replaceWith(document.createTextNode(${JSON.stringify(sellerInitial)}))">`
                        : sellerInitial;
                    // ====== TELEGRAM PREMIUM BADGE ======
                    // Зелёный бейдж в правом верхнем углу карточки.
                    // Появляется только если бэкенд вернул has_premium=true.
                    // Текст «Телеграмм премиум» — как просили в ТЗ.
                    ...
                    card.innerHTML = `
                        ...
                        <div class="card-seller">
                            <div class="card-seller-avatar">${avatarHtml}</div>
            ...
            function openItem(it) {
                ...
                // В модалке больше нет «последних 4 цифр номера» — показываем
                // только страну происхождения. Полный номер покупатель
                // получит уже после оплаты, в «Моих покупках».
                ...
                // Берём first_name + last_name — отображаем целиком, без
                // усечения. Если ничего нет — fallback на username.
                const sellerFirst = (it.seller_first_name || '').trim();
                const sellerLast = (it.seller_last_name || '').trim();
                const fullName = (sellerFirst + (sellerLast ? ' ' + sellerLast : '')).trim();
                const sellerHandle = it.seller_username
    ...
    try:
        ...
        for a in rows:
            ...
            items.append({
                ...
                # описание (из связанного listing'а + дата регистрации)
                "description": build_listing_description(
                    raw_desc, a.reg_month, a.reg_year
                ),
                "title": (listing.title if listing and listing.title else None),
                # Дата регистрации отдельно — нужна фронту для фильтра
                # «Дата создания аккаунта (от/до)», и для будущих фич,
                # где покупатель захочет увидеть дату явно.
