...
def api_buy(telegram_id, tg_user):
    ...
    try:
        ...
        # ===== FIX «уже продан» =====
        # В mini-app прилетали жалобы «пишет аккаунт уже продан, хотя он
        # не продан». Реальная причина — в БД остался is_sold=True без
        # соответствующей записи Purchase (например, бот пометил аккаунт
        # проданным в холостую / сделка отменилась / тестовая покупка).
        # Защита от ложного already_sold: если по аккаунту нет ни одной
        # реальной Purchase — сбрасываем флаг и идём дальше.
        if account.is_sold:
            ...
            if real_purchase is None:
                # Сбой БД: флаг висит без причины. Сбрасываем.
                try:
                    app.logger.warning(
                        "Resetting orphan is_sold on account %s (no real Purchase)",
                        account.id,
                    )
                except Exception:
                    pass
